...
One renderer assembles the final Telegram HTML:

```text
<b>Headline</b>

One short paragraph of 40–60 words.

Source: <a href="URL">Name</a> · <a href="URL">Name</a>

#Tag #Tag #Tag
```

Source URLs are never visible as raw text. Every anchor-bearing Telegram text/caption is required to use `parse_mode=HTML`.
...
Checks include HTTP 200, image MIME type, maximum file size, decodability, Telegram-compatible dimensions, and a reasonable aspect ratio. The original verified file is sent without cropping or resizing. Obvious low-quality/repost hosts are penalized or rejected. If no relevant verified image survives, the bot uses the branded 1200×675 fallback card.
...
Both messages use a Rich H1 and exactly two table columns:

```text
Previous-day results: GAME | RESULT
Next-day games:       GAME | TIME
```
...
- pins the production application version to `1.0.0`
