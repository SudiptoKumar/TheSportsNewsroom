...
V4 calls Exa through its REST API using the supplied evergreen prompt and JSON output schema.
...
EXA_API_KEY
Type: deep
System prompt: prompts/exa_daily_evergreen_v4.txt
Output schema: schemas/exa_daily_sports_games_output_schema_v1.json
Highlights: enabled
Image links: requested
```

Before research, V4 builds a compact exclusion context from `coverage_index.json` and injects it into the prompt so Exa can search around previously covered knowledge.

The local validator still remains authoritative. Exa output is rejected when it violates the sector matrix, completeness, source requirements, evergreen scope, URL requirements, or duplicate rules.
