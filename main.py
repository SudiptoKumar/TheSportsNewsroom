...
def major_live_events(events: list[dict], kind: str, target: date, limit: int = 20) -> list[dict]:
    ...
    for ev in events:
        ...
        if row["major_score"] < LIVE_MAJOR_MIN_SCORE:
            continue
        rows.append(row)
...
def live_rows(events: list[dict], kind: str) -> list[list[dict]]:
    """Build a compact two-column table for the live pair."""
    if kind == "next":
...
def live_rich(kind: str, target: date, events: list[dict]) -> dict:
    ...
    heading = "🏟️ NEXT-DAY MAJOR GAMES" if kind == "next" else "🏅 PREVIOUS-DAY MAJOR RESULTS"
...
def live_pair(state: dict, now: datetime) -> tuple[dict, dict, list[dict], list[dict]]:
    ...
    next_major=major_live_events(next_filtered,"next",tomorrow)
    past_major=major_live_events(past_filtered,"past",yesterday)
    if not next_major:
        logger.warning("No major next-day events found; live schedule will be skipped")
    if not past_major:
        logger.warning("No major previous-day results found; live results will be skipped")
...
def v1_editorialize(ai: "AIClient", candidate: dict, evidence: str) -> dict | None:
    ...
    return ai.json("v1_editorial_post",system,user,EDITORIAL_SCHEMA,max_tokens=max(384,min(700,_env_int("V1_EDITORIAL_MAX_TOKENS",512))),temperature=0.25)


def v1_publish_evergreen(story: dict, media_path: str | None = None) -> dict:
    """Publish one evergreen post with a verified original photo or branded-card fallback."""
    story=v1_normalize_story(story)
    rendered=render_evergreen_post(story)
    chosen_path=media_path if media_path and text(story.get("image_status")) == "verified_external" else None
    temporary=True
    if not chosen_path:
        chosen_path,_=prepare_evergreen_media(story)
    if not chosen_path:
        return {"ok":False,"description":"V1 evergreen publication failed: no verified photo and card generation failed"}
    try:
        payload={
            "chat_id":CHANNEL,
        ...
        return tg_call("sendPhoto",payload,chosen_path)
    finally:
        if temporary:
...
def rotate_live_pair(state: dict, vstate: dict, run_id: str,
    ...
    def publish_or_preview(kind: str, target: date, events: list[dict]) -> dict:
        if dry or not events:
            return {"ok":True,"skipped":not bool(events),"dry_run":dry,"result":{"message_id":None}}
    ...
    sr=publish_or_preview("live_schedule",sched_target,next_events)
    rr=publish_or_preview("live_results",res_target,past_events)
    if not sr.get("ok"):
