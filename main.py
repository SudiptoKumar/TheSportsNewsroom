#!/usr/bin/env python3
# The Sports Newsroom V1 - one production engine for evergreen Sports & Games knowledge and a rotating live pair for @TheSportsNewsroom.
#
# Design (see README.md):
#   V1: Normal web search discovers -> strict sports/evidence gate -> Cerebras ranks -> Exa verifies only as fallback ->
#   optional Exa Agent resolves hard cases -> Cerebras edits -> normalization/validation -> Telegram publishes.
#   Live sports is a rotating two-message rich pair with transactional replacement and cleanup.
#
# Dependencies: requests, Pillow.  Normal web search/image search, Exa fallback, Cerebras and Telegram are called over plain REST.
...
CHANNEL_DISPLAY_NAME = os.environ.get("TELEGRAM_CHANNEL_NAME", "Sports News").strip() or "Sports News"
CHANNEL_URL = os.environ.get("TELEGRAM_CHANNEL_URL", "https://t.me/TheSportsNewsroom").strip() or "https://t.me/TheSportsNewsroom"
CHANNEL_HANDLE = (CHANNEL.strip() if CHANNEL.strip().startswith("@") else "").strip()
...
def _looks_like_hostname(value: str) -> bool:
    ...
    return bool(re.match(r"^(?:[a-z0-9-]+\.)+[a-z]{2,}(?::\d+)?(?:/|$)", raw))
...
def canonical_url(url: str) -> str:
    ...
    if not _looks_like_hostname(parts.netloc):
        return ""
...
def domain_of(url: str) -> str:
    ...
    return host if _looks_like_hostname(parts.netloc) else ""
...
def _canonical_hashtag(raw: Any) -> str:
    value=text(raw).strip()
    if not value:
        return ""
    if value.startswith("#"):
        body=re.sub(r"[^A-Za-z0-9]", "", value[1:])
    else:
        body=re.sub(r"[^A-Za-z0-9]", "", value)
    return "#" + body[:32] if body else ""


def _select_public_source(story: dict) -> tuple[str, str]:
    """Return exactly one human-readable public source for the final post."""
    candidates=[]
    raw_sources=[]
    ...
    if research_sources:
        raw_sources.extend(research_sources)
    public_sources=story.get("sources", []) if isinstance(story.get("sources"), list) else []
    raw_sources.extend(public_sources)
    for idx,item in enumerate(raw_sources):
        if isinstance(item,dict):
            name=text(item.get("name")); url=text(item.get("url")); grade=text(item.get("grade")).upper()
            evidence_len=len(text(item.get("evidence_note")))
        elif isinstance(item,(list,tuple)) and len(item)>=2:
            name=text(item[0]); url=text(item[1]); grade=""; evidence_len=0
        else:
            continue
        url=v1_network_url(url)
        if not url or not re.match(r"^https?://",url,re.I):
            continue
        if not name or re.match(r"^https?://",name,re.I) or name.startswith("a1ahr"):
            name=source_label(url)
        rank=(3 if grade=="A" else 2 if grade=="B" else 1, min(evidence_len,1200), -idx)
        candidates.append((rank,name,url))
    if not candidates:
        raise ValueError("evergreen renderer requires at least one valid source")
    _,name,url=max(candidates,key=lambda x:x[0])
    return name,url


def _public_hashtags(story: dict) -> list[str]:
    """Return exactly two relevant hashtags when possible, never more than two."""
    tags=[]; seen=set()
    for raw in (story.get("tags") or []):
        tag=_canonical_hashtag(raw)
        if tag and tag.lower() not in seen:
            seen.add(tag.lower()); tags.append(tag)
        if len(tags)>=2:
            break
    if len(tags)<2:
        for raw in (story.get("topic"),story.get("sector"),"SportsNews"):
            tag=_canonical_hashtag(raw)
            if tag and tag.lower() not in seen:
                seen.add(tag.lower()); tags.append(tag)
            if len(tags)>=2:
                break
    if not tags:
        tags=["#SportsNews","#SportsGames"]
    elif len(tags)==1:
        tags.append("#SportsGames" if tags[0].lower() != "#sportsgames" else "#SportsNews")
    return tags[:2]


def render_evergreen_post(story: dict) -> dict:
    """Build the only evergreen Telegram HTML payload. Visible order is fixed and minimal."""
    ...
    source_name,source_url=_select_public_source(story)
    tags=_public_hashtags(story)
    active_handle=CHANNEL.strip() if CHANNEL.strip().startswith("@") else ""
    if active_handle:
        effective_channel_url=f"https://t.me/{active_handle[1:]}"
        effective_channel_name=CHANNEL_DISPLAY_NAME if active_handle == "@TheSportsNewsroom" else active_handle.lstrip("@")
    else:
        effective_channel_url=CHANNEL_URL
        effective_channel_name=CHANNEL_DISPLAY_NAME
    channel_link=f'<b><a href="{esc_attr(effective_channel_url)}">{esc(effective_channel_name)}</a></b>'

    # Exact public structure: photo (Telegram media), heading, context, channel + 2 hashtags, one clickable source.
    html_text="\n\n".join([
        f"<b>{esc(headline)}</b>",
        esc(body),
        channel_link + " " + " ".join(esc(tag) for tag in tags),
        "Source: " + f'<a href="{esc_attr(source_url)}">{esc(source_name)}</a>',
    ])
    if vis_len(html_text)>CAPTION_LIMIT:
        raise ValueError("evergreen Telegram HTML exceeds caption limit")
    return {"html":html_text,"parse_mode":"HTML","source_name":source_name,"source_url":source_url,"hashtags":tags}
...
# ===========================================================================
# 5. IMAGE MEDIA (verified external photos with branded-card fallback)
# ===========================================================================
...
IMAGE_SEARCH_NOISE = {"logo", "icon", "avatar", "favicon", "sprite", "wallpaper", "illustration", "vector", "clipart", "poster", "template"}
IMAGE_ALLOWED_MIME = {"image/jpeg", "image/png", "image/webp"}


def _image_story_query(story: dict) -> str:
    parts=[text(story.get("headline")),text(story.get("normalized_subject")),text(story.get("central_knowledge_unit")),text(story.get("topic"))]
    people=story.get("people") if isinstance(story.get("people"),list) else []
    parts.extend(text(x) for x in people[:4] if text(x))
    if text(story.get("sector")): parts.append(text(story.get("sector")))
    return re.sub(r"\s+"," "," ".join(x for x in parts if x)).strip()[:420]

def _image_query_variants(story: dict) -> list[str]:
    base=_image_story_query(story)
    subject=text(story.get("normalized_subject") or story.get("headline"))
    people=[text(x) for x in (story.get("people") or []) if text(x)] if isinstance(story.get("people"),list) else []
    historical=any(k in normalize_text(story.get("sector")) for k in ("history","origin","game"))
    variants=[f"{base} exact subject photo sports",f"{subject} official photo"+(f" {people[0]}" if people else ""),f"{subject} historical photograph" if historical else f"{subject} match photo"]
    out=[]; seen=set()
    for q in variants:
        q=re.sub(r"\s+"," ",q).strip()[:420]
        if q and q.lower() not in seen: seen.add(q.lower()); out.append(q)
    return out


def _image_query_tokens(story: dict) -> set[str]:
    return tokens(" ".join([
        text(story.get("headline")),
        text(story.get("normalized_subject")),
        text(story.get("central_knowledge_unit")),
        text(story.get("topic")),
        " ".join(text(x) for x in (story.get("people") or [])[:4]) if isinstance(story.get("people"), list) else "",
    ]))


def _image_relevance(story: dict, candidate: dict) -> float:
    wanted=_image_query_tokens(story)
    hay=" ".join(text(candidate.get(k)) for k in ("title","alt","page_title","page_url","source_name","image_url","image_alt"))
    got=tokens(hay)
    if not wanted or not got: return 0.0
    overlap=len(wanted & got)/max(1,len(wanted))
    hay_norm=normalize_text(hay)
    subject=normalize_text(story.get("normalized_subject") or story.get("headline"))
    exact=0.45 if subject and len(subject)>=8 and subject in hay_norm else 0.0
    people=[normalize_text(x) for x in (story.get("people") or []) if text(x)] if isinstance(story.get("people"),list) else []
    person_bonus=0.25 if any(p and p in hay_norm for p in people) else 0.0
    source_bonus=0.10 if candidate.get("provider")=="source_page" else 0.0
    return min(1.0,overlap*0.60+exact+person_bonus+source_bonus)

def _image_is_probable_placeholder(path: str) -> bool:
    if Image is None: return False
    try:
        with Image.open(path) as im:
            rgb=im.convert("RGB"); sample=rgb.copy(); sample.thumbnail((96,96)); px=list(sample.get_flattened_data() if hasattr(sample,"get_flattened_data") else sample.getdata())
            if not px: return True
            total=len(px); bright=sum(1 for r,g,b in px if r>=248 and g>=248 and b>=248)/total; dark=sum(1 for r,g,b in px if r<=7 and g<=7 and b<=7)/total
            if bright>=0.985 or dark>=0.985: return True
            mean=sum(0.2126*r+0.7152*g+0.0722*b for r,g,b in px)/total
            variance=sum((0.2126*r+0.7152*g+0.0722*b-mean)**2 for r,g,b in px)/total
            return variance<0.5 and (mean>248 or mean<7)
    except Exception:
        return True


def _image_penalty(candidate: dict) -> float:
    hay = normalize_text(" ".join([
        text(candidate.get("title")), text(candidate.get("alt")),
        text(candidate.get("image_url")), text(candidate.get("page_url")),
    ]))
    d=domain_of(text(candidate.get("page_url") or candidate.get("image_url")))
    blocked=any(d==bad or d.endswith("."+bad) for bad in IMAGE_BLOCKED_DOMAINS)
    base=sum(0.10 for term in IMAGE_SEARCH_NOISE if term in hay)
    return min(1.0, base + (0.50 if blocked else 0.0))


def _parse_bing_image_results(raw_html: str, limit: int = 10) -> list[dict]:
    """Parse Bing Images result anchors and keep original image URLs, not thumbnails."""
    out: list[dict] = []
    seen: set[str] = set()
    for tag in re.findall(r"<a\b[^>]*\bclass=[\"'][^\"']*\biusc\b[^\"']*[\"'][^>]*>", raw_html or "", flags=re.I | re.S):
        m = re.search(r"\bm=([\"'])(.*?)\1", tag, flags=re.I | re.S)
        if not m:
            continue
        raw_meta = html.unescape(m.group(2))
        try:
            meta = json.loads(raw_meta)
        except Exception:
            meta = {}
            mm = re.search(r'\"murl\"\s*:\s*\"(.*?)\"', raw_meta)
            pp = re.search(r'\"purl\"\s*:\s*\"(.*?)\"', raw_meta)
            tt = re.search(r'\"t\"\s*:\s*\"(.*?)\"', raw_meta)
            if mm: meta["murl"] = bytes(mm.group(1), "utf-8").decode("unicode_escape")
            if pp: meta["purl"] = bytes(pp.group(1), "utf-8").decode("unicode_escape")
            if tt: meta["t"] = bytes(tt.group(1), "utf-8").decode("unicode_escape")
        image_url = v1_network_url(text(meta.get("murl")))
        page_url = v1_network_url(text(meta.get("purl")))
        ...
        out.append({
            "image_url": image_url,
            "page_url": page_url,
            "title": text(meta.get("t") or meta.get("desc")),
            "page_title": text(meta.get("t") or meta.get("desc")),
            "alt": text(meta.get("desc")),
            "image_alt": text(meta.get("desc")),
            "source_name": source_label(page_url) if page_url else domain_of(image_url),
...
def normal_image_search(query: str, *, num: int = 10) -> list[dict]:
    """Primary no-API-key image discovery using ordinary Bing Images search."""
    if not text(query):
        return []
    r = http(
        "normal-image-search:bing",
        "GET",
        "https://www.bing.com/images/search",
        params={"q": query, "form": "HDRSC2", "first": "1", "count": str(num), "setlang": "en-US"},
    ...
    if not r.ok or not r.text:
        logger.warning("Normal image search failed status=%s error=%s", r.status, redact(r.error)[:180])
        return []
    return _parse_bing_image_results(r.text, num)


def _extract_meta_image_candidates(page_url: str, raw_html: str) -> list[dict]:
    """Extract lead-image URLs plus page identity/description for image relevance scoring."""
    raw=raw_html or ""
    def meta(name: str) -> str:
        ...
        for pat in pats:
            ...
            if m: return html.unescape(text(m.group(1)))
        return ""
    ...
    page_title=meta("og:title") or meta("twitter:title") or title or page_url
    description=meta("og:description") or meta("description") or meta("twitter:description")
    site_name=meta("og:site_name") or meta("application-name") or source_public_name(page_url)
    out=[]; seen=set()
    patterns=[
        (r'<meta[^>]+(?:property|name)=["\']og:image(?::secure_url)?["\'][^>]+content=["\']([^"\']+)["\']',"og:image"),
        (r'<meta[^>]+(?:property|name)=["\']twitter:image(?::src)?["\'][^>]+content=["\']([^"\']+)["\']',"twitter:image"),
        (r'<meta[^>]+itemprop=["\']image["\'][^>]+content=["\']([^"\']+)["\']',"itemprop:image"),
        (r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+(?:property|name)=["\']og:image(?::secure_url)?["\']',"og:image"),
        (r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+(?:property|name)=["\']twitter:image(?::src)?["\']',"twitter:image"),
    ]
    def add(u: str, kind: str):
        key=canonical_url(u)
        if not key or key in seen: return
        seen.add(key); out.append({"image_url":u,"page_url":page_url,"title":page_title,"page_title":page_title,"alt":description or kind,"image_alt":description,"source_name":site_name,"provider":"source_page"})
    for rx,kind in patterns:
        for match in re.findall(rx,raw,flags=re.I|re.S):
            u=v1_network_url(urljoin(page_url,html.unescape(text(match))))
            if u: add(u,kind)
    for block in re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>',raw,flags=re.I|re.S):
        block=html.unescape(block)
        publisher=re.search(r'"publisher"\s*:\s*\{.*?"name"\s*:\s*"([^"]+)"',block,flags=re.I|re.S)
        if publisher and site_name==source_public_name(page_url): site_name=text(publisher.group(1))
        for match in re.findall(r'"image"\s*:\s*(?:"([^\"]+)"|\[\s*"([^\"]+)")',block,flags=re.I|re.S):
            raw_image=text(next((x for x in match if x),""))
            u=v1_network_url(urljoin(page_url,raw_image))
            if u: add(u,"json-ld:image")
    return out


def _fetch_source_page_head(page_url: str) -> list[dict]:
    if not page_url or requests is None:
        return []
    try:
        r = session().get(page_url, headers={**HEADERS, "Accept": "text/html,application/xhtml+xml"}, timeout=IMAGE_TIMEOUT, allow_redirects=True, stream=True)
        if int(getattr(r, "status_code", 0) or 0) != 200:
            return []
        ctype = text(getattr(r, "headers", {}).get("Content-Type"))
        ...
        for chunk in r.iter_content(chunk_size=64*1024):
            ...
            total += len(chunk)
            if total > IMAGE_PAGE_MAX_BYTES: break
            chunks.append(chunk)
        raw=b"".join(chunks).decode("utf-8", errors="ignore")
        return _extract_meta_image_candidates(text(getattr(r, "url", "")) or page_url, raw)
    except Exception as exc:
        logger.info("Source-page image metadata unavailable %s: %s", domain_of(page_url), redact(str(exc))[:120])
        return []


def discover_evergreen_image_candidates(story: dict) -> list[dict]:
    """Queue source-page leads plus multiple targeted image searches, ranked for exact relevance."""
    candidates=[]; seen=set(); source_pages=[]
    for item in story.get("sources",[]) if isinstance(story.get("sources"),list) else []:
        if isinstance(item,(list,tuple)) and len(item)>=2 and text(item[1]): source_pages.append(v1_network_url(text(item[1])))
        elif isinstance(item,dict) and text(item.get("url")): source_pages.append(v1_network_url(text(item.get("url"))))
    source_pages=list(dict.fromkeys(x for x in source_pages if x))
    for page in source_pages[:3]:
        for c in _fetch_source_page_head(page)[:4]:
            key=canonical_url(c.get("image_url"))
            if key and key not in seen:
                seen.add(key); c["score"]=72+_image_relevance(story,c)*25-_image_penalty(c)*20; candidates.append(c)
    source_domains={domain_of(x) for x in source_pages}
    for query in _image_query_variants(story):
        for c in normal_image_search(query,num=IMAGE_SEARCH_RESULTS):
            key=canonical_url(c.get("image_url"))
            if not key or key in seen: continue
            seen.add(key); relevance=_image_relevance(story,c); domain_bonus=18 if domain_of(c.get("page_url")) in source_domains else 0
            c["score"]=45+domain_bonus+relevance*42-_image_penalty(c)*25; c["query"]=query; candidates.append(c)
    candidates.sort(key=lambda x:float(x.get("score",0)),reverse=True)
    return candidates[:24]


def download_verified_image(candidate: dict) -> dict | None:
    """Download and fully verify a candidate image before uploading it to Telegram."""
    url=v1_network_url(text(candidate.get("image_url")))
    if not url or requests is None or Image is None: return None
    headers={**HEADERS,"Accept":"image/avif,image/webp,image/apng,image/*,*/*;q=0.8"}
    referer=v1_network_url(text(candidate.get("page_url")))
    if referer: headers["Referer"]=referer
    raw_path=""
    try:
        h=None
        try: h=session().head(url,headers=headers,timeout=IMAGE_TIMEOUT,allow_redirects=True)
        except Exception: pass
        hstatus=int(getattr(h,"status_code",0) or 0) if h else 0; htype=text(getattr(h,"headers",{}).get("Content-Type")).split(";",1)[0].lower() if h else ""; hlen=text(getattr(h,"headers",{}).get("Content-Length")) if h else ""
        if hstatus==200 and hlen.isdigit() and int(hlen)>MAX_IMAGE_BYTES: return None
        if hstatus==200 and htype in {"image/gif","image/svg+xml"}: return None
        get_url=text(getattr(h,"url","")) or url
        r=session().get(get_url,headers=headers,timeout=IMAGE_TIMEOUT,allow_redirects=True,stream=True)
        if int(getattr(r,"status_code",0) or 0)!=200: return None
        ctype=text(getattr(r,"headers",{}).get("Content-Type")).split(";",1)[0].lower()
        if ctype not in IMAGE_ALLOWED_MIME: return None
        length=text(getattr(r,"headers",{}).get("Content-Length"))
        if length.isdigit() and int(length)>MAX_IMAGE_BYTES: return None
        fd,raw_path=tempfile.mkstemp(prefix="sn_photo_raw_",suffix=Path(urlsplit(getattr(r,"url",get_url)).path).suffix[:8] or ".img"); os.close(fd)
        total=0
        with open(raw_path,"wb") as fh:
            for chunk in r.iter_content(chunk_size=64*1024):
                if not chunk: continue
                total+=len(chunk)
                if total>MAX_IMAGE_BYTES: Path(raw_path).unlink(missing_ok=True); return None
                fh.write(chunk)
        if total<1024: Path(raw_path).unlink(missing_ok=True); return None
        with Image.open(raw_path) as im:
            width,height=im.size; fmt=text(im.format).upper()
            if width<IMAGE_MIN_WIDTH or height<IMAGE_MIN_HEIGHT or width>12000 or height>12000: Path(raw_path).unlink(missing_ok=True); return None
            ratio=width/max(1,height)
            if ratio<IMAGE_MIN_ASPECT or ratio>IMAGE_MAX_ASPECT: Path(raw_path).unlink(missing_ok=True); return None
            if fmt not in {"JPEG","PNG","WEBP"}: Path(raw_path).unlink(missing_ok=True); return None
            im.verify()
        if _image_is_probable_placeholder(raw_path): Path(raw_path).unlink(missing_ok=True); return None
        final_path=raw_path
        with Image.open(raw_path) as im:
            if text(im.format).upper()!="JPEG" or im.mode not in {"RGB","L"}:
                rgb=im.convert("RGB"); fd2,final_path=tempfile.mkstemp(prefix="sn_photo_",suffix=".jpg"); os.close(fd2); rgb.save(final_path,"JPEG",quality=92,optimize=True); rgb.close(); Path(raw_path).unlink(missing_ok=True)
                with Image.open(final_path) as check: check.verify(); width,height=check.size
        return {"path":final_path,"url":text(getattr(r,"url",get_url)) or get_url,"source_page_url":text(candidate.get("page_url")),"source_name":source_public_name(text(candidate.get("page_url") or get_url),text(candidate.get("source_name"))),"provider":text(candidate.get("provider")),"bytes":Path(final_path).stat().st_size,"width":width,"height":height,"score":float(candidate.get("score",0))}
    except Exception as exc:
        if raw_path: Path(raw_path).unlink(missing_ok=True)
        logger.info("Image verification failed url=%s: %s",domain_of(url),redact(str(exc))[:160])
        return None


def prepare_evergreen_media(story: dict, *, allow_exa_fallback: bool = True) -> tuple[str, dict]:
    """Select a relevant photo using normal search first, verify/download it, then optionally fall back to Exa and finally a card.

    ``allow_exa_fallback`` is explicit so production keeps the paid recovery path while
    deterministic/offline callers can disable external Exa image discovery without
    mutating global configuration or leaking credentials into test behavior.
    """
    candidates=discover_evergreen_image_candidates(story)
    for candidate in candidates:
        rel=_image_relevance(story,candidate)
        penalty=_image_penalty(candidate)
        if penalty >= 0.20:
            continue
        if rel < IMAGE_MIN_RELEVANCE and candidate.get("provider") != "source_page":
            continue
        verified=download_verified_image(candidate)
        if verified:
            aspect=verified["width"]/max(1,verified["height"])
            if aspect < IMAGE_MIN_ASPECT or aspect > IMAGE_MAX_ASPECT:
                logger.info("Image rejected geometry aspect=%.2f source=%s",aspect,domain_of(verified["url"]))
                try: os.remove(verified["path"])
                except OSError: pass
                continue
            story["image"]={k:verified[k] for k in ("url","source_page_url","source_name","provider","bytes","width","height","score")}
            story["image_status"]="verified_external"
            logger.info("Verified story photo selected provider=%s size=%sx%s bytes=%s source=%s",verified["provider"],verified["width"],verified["height"],verified["bytes"],domain_of(verified["url"]))
            return verified["path"], story["image"]

    # Paid Exa image discovery is used only after normal image/source-page candidates were all rejected or failed verification.
    if allow_exa_fallback and EXA_API_KEY:
        query=_image_story_query(story) + " exact subject photo image"
        for c in v1_exa_search_query(query, now_bd().date(), set(), mode="auto", num=5):
            image_url=text(c.get("image"))
            if not image_url:
                continue
            candidate={"image_url":image_url, "page_url":text(c.get("url")), "title":text(c.get("title")), "alt":text(c.get("text")), "source_name":text(c.get("source")), "provider":"exa_image_fallback", "score":30 + (_image_relevance(story,c)*30) - (_image_penalty(c)*25)}
            if _image_penalty(candidate) >= 0.20 or _image_relevance(story,candidate) < IMAGE_MIN_RELEVANCE:
                continue
            verified=download_verified_image(candidate)
            if verified:
                story["image"]={k:verified[k] for k in ("url","source_page_url","source_name","provider","bytes","width","height","score")}
                story["image_status"]="verified_external"
                logger.info("Verified story photo selected through Exa fallback size=%sx%s bytes=%s source=%s",verified["width"],verified["height"],verified["bytes"],domain_of(verified["url"]))
                return verified["path"], story["image"]

    card=make_card(story)
    story["image"]={}
    story["image_status"]="card_fallback"
    logger.warning("No verified relevant story photo found; using branded card")
    return card, {"mode":"card_fallback"}
...
def _card_info_items(story: dict) -> list[tuple[str, str]]:
    """Return at most three high-value metadata blocks for the branded card."""
    research=story.get("research") if isinstance(story.get("research"),dict) else {}
    candidates=[
        ("KEY FACT", text(story.get("central_claim") or research.get("central_claim"))),
        ("WHEN", text(story.get("historical_date") or story.get("historical_year") or research.get("historical_date") or research.get("historical_year"))),
        ("WHERE", text(story.get("region") or story.get("country") or research.get("region") or research.get("country"))),
    ]
    out=[]
    for label,value in candidates:
        value=re.sub(r"\s+", " ", value).strip()
        if not value: continue
        if value.lower() in {"unknown","n/a","none"}: continue
        out.append((label,value[:96]))
        if len(out)>=3: break
    if not out:
        out=[("TOPIC", text(story.get("topic") or story.get("normalized_subject"))[:96])]
    return out


def make_card(story: dict) -> str:
    """Create one consistent 1200x675 branded card with a headline and up to three info columns."""
    if Image is None:
        return ""
    try:
        W,H=1200,675
        ...
        img=Image.new("RGB",(W,H),top); draw=ImageDraw.Draw(img)
        for y in range(H):
            t=y/(H-1)
            draw.line([(0,y),(W,y)],fill=tuple(int(top[i]+(bot[i]-top[i])*t) for i in range(3)))
        # Subtle sports motif. Keep the central content area clean.
        for k,rad in enumerate((260,205,150,95)):
            cx,cy=W-175,125
            draw.ellipse([cx-rad,cy-rad,cx+rad,cy+rad],outline=tuple(min(255,v+28+k*5) for v in top),width=5)
        label=text(story.get("label") or FORMAT_LABELS.get(fmt,"SPORTS & GAMES"))
        f_label=card_font(28)
        lw=int(draw.textlength(label,font=f_label))
        draw.rounded_rectangle([58,48,58+lw+50,106],radius=27,fill=(255,255,255))
        draw.text((83,57),label,font=f_label,fill=top)

        headline=text(story.get("headline")) or "The Sports Newsroom"
        chosen=None
        for size in (66,58,52,46,42):
            f=card_font(size); lines=_wrap(draw,headline,f,W-160)
            if len(lines)<=3:
                chosen=(size,f,lines); break
        if chosen is None:
            size=42; f=card_font(size); lines=_wrap(draw,clamp_words(headline,90),f,W-160)[:3]
        else:
            size,f,lines=chosen
        y=150
        for line in lines:
            bbox=draw.textbbox((0,0),line,font=f); lh=bbox[3]-bbox[1]
            draw.text((66,y+3),line,font=f,fill=(0,0,0))
            draw.text((64,y),line,font=f,fill=(255,255,255))
            y+=int(max(lh,size)*1.16)

        items=_card_info_items(story)
        box_gap=22
        box_x=60
        box_w=(W-120-box_gap*(len(items)-1))//len(items)
        box_y=455; box_h=112
        f_k=card_font(22); f_v=card_font(25)
        for i,(k,v) in enumerate(items):
            x=box_x+i*(box_w+box_gap)
            fill=tuple(min(255,vv+20) for vv in top)
            draw.rounded_rectangle([x,box_y,x+box_w,box_y+box_h],radius=18,fill=fill,outline=(255,255,255),width=2)
            draw.text((x+18,box_y+14),k,font=f_k,fill=(225,235,250))
            lines=_wrap(draw,v,f_v,box_w-36)[:2]
            vy=box_y+46
            for line in lines:
                draw.text((x+18,vy),line,font=f_v,fill=(255,255,255)); vy+=30

        handle=CHANNEL if CHANNEL.startswith("@") else "@TheSportsNewsroom"
        f_handle=card_font(26)
        draw.text((W-60-int(draw.textlength(handle,font=f_handle)),H-42),handle,font=f_handle,fill=(255,255,255))
...
class AIClient:
    ...
    MODEL_PREFS = ["gpt-oss-120b"]
    MIN_INTERVAL = max(5.0, _env_float("CEREBRAS_MIN_INTERVAL_SECONDS", 12.5))
    MAX_CALLS_PER_RUN = max(1, _env_int("CEREBRAS_MAX_CALLS_PER_RUN", 16))
    RATE_LIMIT_FALLBACK_SECONDS = max(30.0, _env_float("CEREBRAS_RATE_LIMIT_FALLBACK_SECONDS", 60.0))
    RATE_LIMIT_MAX_PAUSE_SECONDS = max(60.0, _env_float("CEREBRAS_RATE_LIMIT_MAX_PAUSE_SECONDS", 300.0))
    ...
    def _payload(self, messages: list, schema: dict, task: str, max_tokens: int, temperature: float) -> dict:
        ...
        if self.reasoning and "gpt-oss" in self.model:
            payload["reasoning_effort"] = "medium" if "rank" in task.lower() else "low"
    ...
    def _switch_model(self) -> bool:
        ...
        for m in models:
            ...
            if bool(m.get("deprecated")):
                continue
            if isinstance(caps,dict) and caps.get("structured_outputs") is False:
                continue
        ...
        for pref in self.MODEL_PREFS:
            if pref in eligible:
                ...
                return True
        if eligible:
            logger.warning("Cerebras model %s unavailable; switching to available structured model %s",self.model,eligible[0])
            self.model=eligible[0]
            return True
        return False
    ...
    def _json(self, task: str, system: str, user: str, schema: dict, *, max_tokens: int = 3000,
        ...
        for attempt in range(5):
            ...
            if r.status == 429:
                ...
                quota_exceeded = any(token in low for token in (
                    "token_quota_exceeded", "too_many_tokens_error", "tokens per minute", "quota exceeded"
                ))
                if quota_exceeded:
                    self.fatal = True
                    REPORT.errors.append(f"Cerebras token quota exhausted: {self.last_error}")
                    logger.warning("Cerebras token quota exhausted; stopping AI for this run: %s", self.last_error)
                    return None
                provider_wait = 0.0
                for key in ("retry-after", "x-ratelimit-reset-tokens-minute", "x-ratelimit-reset-requests-minute"):
                    raw_wait = text(r.headers.get(key) or self.rate_limits.get(key))
                    try:
                        value=float(raw_wait.rstrip("s")) if raw_wait else 0.0
                    except ValueError:
                        value=0.0
                    provider_wait=max(provider_wait,value)
                fallback_wait=min(self.RATE_LIMIT_MAX_PAUSE_SECONDS, self.RATE_LIMIT_FALLBACK_SECONDS * (2 ** max(0, self.rate_limit_streak - 1)))
                wait=max(self.RATE_LIMIT_FALLBACK_SECONDS, provider_wait, fallback_wait)
                self._throttle_until=max(self._throttle_until, time.monotonic()+wait)
                logger.warning("Cerebras rate limited (attempt %d/5, streak=%d); retrying after %.1fs", attempt+1, self.rate_limit_streak, wait)
                if attempt < 4 and not self.fatal:
                    sleep(wait)
                    continue
            ...
            if r.status == 402:
                self.fatal = True
                self.last_error = self._error_detail(r)
                REPORT.errors.append(f"Cerebras payment/credit failure: {self.last_error}")
                logger.warning("Cerebras credits/payment required; stopping AI for this run: %s", self.last_error)
                return None
...
SOURCE_LABELS = {
    ...
    "theathletic.com": "The Athletic", "cbssports.com": "CBS Sports", "foxsports.com": "Fox Sports",
    "nbcsports.com": "NBC Sports", "si.com": "Sports Illustrated", "fivb.com": "FIVB",
    "fiba.basketball": "FIBA", "nba.com": "NBA", "mlb.com": "MLB", "nfl.com": "NFL", "nhl.com": "NHL",
    "atptour.com": "ATP Tour", "wtatennis.com": "WTA", "pgatour.com": "PGA Tour",
    "motorsport.com": "Motorsport.com", "autosport.com": "Autosport", "autoweek.com": "Autoweek",
}

_SOURCE_NAME_CACHE: dict[str,str] = {}

def _humanize_domain(d: str) -> str:
    host=text(d).removeprefix("www.").split(".")[0]
    if not host: return "Source"
    known={"fivb":"FIVB","fiba":"FIBA","nba":"NBA","mlb":"MLB","nfl":"NFL","nhl":"NHL","wta":"WTA","atp":"ATP Tour","ufc":"UFC","pga":"PGA Tour"}
    return known.get(host.lower()," ".join(x.capitalize() for x in re.sub(r"[-_]+"," ",host).split()))

def source_public_name(url: str, fallback: str = "") -> str:
    d=domain_of(url)
    for dom,label in SOURCE_LABELS.items():
        if d==dom or d.endswith("."+dom): return label
    fb=text(fallback).strip()
    if fb and not _looks_like_hostname(fb) and not re.match(r"^https?://",fb,re.I) and len(fb)<=90 and not fb.startswith("a1ahr") and not re.fullmatch(r"[A-Za-z0-9_-]{40,}",fb): return fb
    return _SOURCE_NAME_CACHE.setdefault(d,_humanize_domain(d)) if d else "Source"
...
_BAD_SOURCE_DOMAINS = {
    "merriam-webster.com", "vocabulary.com", "thefreedictionary.com", "dictionary.com",
    "thesaurus.com", "cambridge.org", "collinsdictionary.com", "yourdictionary.com",
    "google.com", "play.google.com", "awardwallet.com", "slideserve.com", "studocu.com",
    "pinterest.com", "pinimg.com",
}
_DEFINITION_TITLE_RX = re.compile(
    r"(?:^|\b)(?:definition|meaning|synonyms?|thesaurus|dictionary|translation|what does|what is the meaning of)\b", re.I
)
_SPORT_CONTEXT_RX = re.compile(
    r"\b(?:sport(?:s)?|athlete|player|coach|referee|football|soccer|basketball|baseball|cricket|rugby|tennis|"
    r"hockey|golf|volleyball|badminton|athletics|swimming|cycling|boxing|wrestling|fencing|archery|rowing|"
    r"motorsport|formula 1|olympic|olympics|paralympic|championship|tournament|league|match|ballgame|"
    r"board game|card game|tabletop|dice game|traditional game|regional game|sporting)\b", re.I
)
_PEOPLE_CONTEXT_RX = re.compile(r"\b(?:athlete|player|coach|referee|founder|inventor|designer|pioneer|champion|organizer|rule[- ]maker)\b", re.I)

def source_label(url: str, fallback: str = "") -> str:
    return source_public_name(url,fallback)


def _source_records(sources: Any) -> list[dict]:
    out=[]; seen=set()
    for item in sources if isinstance(sources,list) else []:
        if isinstance(item,dict):
            raw_url=text(item.get("url")); raw_name=text(item.get("name"))
            typ=text(item.get("type")); note=text(item.get("evidence_note"))
        elif isinstance(item,(list,tuple)) and len(item)>=2:
            raw_name=text(item[0]); raw_url=text(item[1]); typ=""; note=""
        else:
            continue
        url=v1_network_url(raw_url)
        cu=canonical_url(url)
        if not url or not cu or cu in seen:
            continue
        seen.add(cu)
        out.append({
            "name": source_public_name(url, raw_name),
            "url": url,
            "type": typ,
            "grade": grade_of(url),
            "evidence_note": note,
...
def evergreen_scope_reason(candidate: dict) -> str:
    url=v1_network_url(text(candidate.get("source_url") or candidate.get("url")))
    if not url:
        return "invalid_source_url"
    d=domain_of(url)
    if any(d == bad or d.endswith("."+bad) for bad in _BAD_SOURCE_DOMAINS):
        return f"blocked_source_domain:{d}"
    title=text(candidate.get("normalized_subject") or candidate.get("title"))
    excerpt=text(candidate.get("text")) + " " + " ".join(text(x) for x in (candidate.get("highlights") or []))
    if _DEFINITION_TITLE_RX.search(title):
        return "dictionary_or_definition"
    blob=f"{title} {excerpt} {url}"
    if exclusion_reason(blob):
        return exclusion_reason(blob)
    sector=text(candidate.get("sector"))
    sportish=bool(_SPORT_CONTEXT_RX.search(blob))
    peopleish=bool(_PEOPLE_CONTEXT_RX.search(blob))
    trusted=(text(candidate.get("grade")).upper() in {"A","B"}) or any(d == dom or d.endswith("."+dom) for dom in (OFFICIAL_DOMAINS+SECONDARY_DOMAINS+RULES_PUBLISHERS+TABLETOP_DOMAINS+REFERENCE_DOMAINS))
    if not sportish and not (peopleish and "People" in sector) and not (trusted and ("Game" in sector or "Sport" in sector)):
        return "not_sports_related"
    return ""
...
def collect_events(ai: "AIClient", target: date, kind: str, *, allow_exa_fallback: bool = True) -> tuple[list[dict], list[str]]:
    ...
    current=filter_state(events, kind, target)
    sports_present={text(e.get("sport")) for e in current if text(e.get("sport"))}
    if len(current) < 12 or len(sports_present) < 3:
        extra=tsdb_events(target)
        notes.append(f"thesportsdb: +{len(extra)} candidate events")
        events=merge_events(events,extra)
    if len(filter_state(events, kind, target)) < 4 and allow_exa_fallback and EXA_API_KEY and ai.available:
...
IMAGE_SEARCH_RESULTS = max(6, _env_int("IMAGE_SEARCH_RESULTS", 8))
IMAGE_SEARCH_TIMEOUT = max(8, _env_int("IMAGE_SEARCH_TIMEOUT", 12))
IMAGE_PAGE_MAX_BYTES = max(64_000, _env_int("IMAGE_PAGE_MAX_BYTES", 512_000))
IMAGE_MIN_RELEVANCE = _env_float("IMAGE_MIN_RELEVANCE", 0.28)
IMAGE_MIN_WIDTH = max(320,_env_int("IMAGE_MIN_WIDTH",320))
IMAGE_MIN_HEIGHT = max(180,_env_int("IMAGE_MIN_HEIGHT",180))
IMAGE_MIN_ASPECT = max(0.5, _env_float("IMAGE_MIN_ASPECT", 0.60))
IMAGE_MAX_ASPECT = min(3.0, _env_float("IMAGE_MAX_ASPECT", 2.00))
IMAGE_BLOCKED_DOMAINS = {"pinterest.com","pinimg.com","slideserve.com","studocu.com","alphacoders.com"}
...
def build_evergreen(candidate: dict, editorial: dict) -> dict:
    ...
    seen=set(); tags=[t for t in tags if t and not (t in seen or seen.add(t))][:2]
    if len(tags) < 2:
        fallback_tags=["#SportsGames", "#SportsHistory"]
        for ft in fallback_tags:
            if ft not in tags:
                tags.append(ft)
            if len(tags) >= 2:
                break
    src=_source_records(candidate.get("sources"))
    sources=[(x["name"],x["url"]) for x in src]
    all_sources=[x for x in src if x.get("url")]
    # Keep all research sources internally, but expose/select exactly one public source in the renderer.
    public_source=min(all_sources, key=lambda x: (0 if text(x.get("grade")).upper()=="A" else 1 if text(x.get("grade")).upper()=="B" else 2, -len(text(x.get("evidence_note"))))) if all_sources else None
    public_sources=[(public_source["name"],public_source["url"])] if public_source else []
    return {
        "desk": "evergreen_v1",
        ...
        "research_sources": [(x["name"],x["url"]) for x in all_sources],
        "urls": [b for a,b in public_sources if b],
...
def live_event_identity(ev: dict) -> str:
    ...
    if home and away:
        ...
        return fingerprint(sport, day, participants)
    return fingerprint(sport, league, stage, day, normalize_text(ev.get("name")))

LIVE_MAJOR_MIN_SCORE = max(0, _env_int("LIVE_MAJOR_MIN_SCORE", 60))

def _live_result_value(event: dict) -> str:
    if event.get("style") == "team" and event.get("home") and event.get("away"):
        hs=text(event.get("home_score")); aw=text(event.get("away_score"))
        if hs != "" and aw != "":
            return f"{hs}–{aw}"
        return ""
    return text(event.get("winner") or "")

def major_live_events(events: list[dict], kind: str, target: date, limit: int = 20, min_score: int | None = None) -> list[dict]:
    rows=[]; seen=set()
    for ev in events:
        if not isinstance(ev,dict): continue
        if kind == "past" and not _live_result_value(ev):
            continue
        eid=live_event_identity(ev)
        if eid in seen: continue
        seen.add(eid)
        row=dict(ev); row["event_id"]=eid; row["major_score"]=live_event_importance(row,kind)
        threshold=LIVE_MAJOR_MIN_SCORE if min_score is None else max(0, int(min_score))
        if row["major_score"] < threshold:
            continue
        rows.append(row)
    ...
    sports_count=len({x.get("sport") for x in rows})
    for e in rows:
        if len(selected)>=limit: break
        sport=text(e.get("sport")) or "Sport"
        cap=5 if sports_count>3 else 6
        if by_sport[sport]>=cap: continue
        selected.append(e); by_sport[sport]+=1
    return selected


def _live_game_name(event: dict) -> str:
    if event.get("style") == "team" and event.get("home") and event.get("away"):
        return f"{text(event.get('home'))} vs {text(event.get('away'))}"
    return text(event.get("name") or event.get("home") or event.get("away") or "Game")

def _compact_live_value(value: str, limit: int = 44) -> str:
...
def live_rows(events: list[dict], kind: str) -> list[list[dict]]:
    value_header="TIME" if kind=="next" else "RESULT"
    rows=[[table_cell("SPORT",True),table_cell("GAME",True),table_cell(value_header,True,"center")]]
    for e in events[:10]:
        sport=_compact_live_value(e.get("sport") or "Sport",20); game=_compact_live_value(_live_game_name(e),48)
        if kind=="next":
            value=to_bd(e["start"]).strftime("%H:%M") if e.get("start") else "TBC"
            rows.append([table_cell(sport),table_cell(game),table_cell(value,align="center")])
        else:
            result=_live_result_value(e)
            if result: rows.append([table_cell(sport),table_cell(game),table_cell(_compact_live_value(result,18),align="center")])
    if len(rows)==1:
        rows.append([table_cell("",align="center"),table_cell("No major games found" if kind=="next" else "No major results found"),table_cell("—",align="center")])
    return rows

def live_rich(kind: str, target: date, events: list[dict]) -> dict:
    heading="🏟️ NEXT-DAY GAME SCHEDULE" if kind=="next" else "🏅 PREVIOUS-DAY RESULTS"
    return {"blocks":[{"type":"heading","size":1,"text":heading},{"type":"paragraph","text":long_date(target)},{"type":"table","cells":live_rows(events,kind),"is_bordered":True,"is_striped":True,"is_compact":True}]}
...
def _unique_message_ids(values: Iterable[Any]) -> list[int]:
    out=[]; seen=set()
    for value in values:
        try:
            mid=int(value)
...
def cleanup_live_retirements(vstate: dict, *, exclude_ids: Iterable[int] = ()) -> bool:
    """Delete stale live messages; retain failures for retry on the next run."""
    live=vstate.setdefault("live", _v1_default_state()["live"])
    pending=_unique_message_ids(live.get("retiring_message_ids", []))
    excluded=set(_unique_message_ids(exclude_ids))
    pending=[mid for mid in pending if mid not in excluded]
    if not pending:
        live["retiring_message_ids"]=[]
        return True
    payload={"chat_id":CHANNEL, "message_ids":json.dumps(pending,separators=(",",":"))}
    res=tg_call("deleteMessages", payload)
    if res.get("ok"):
        live["retiring_message_ids"]=[]
        logger.info("Live pair cleanup complete: deleted %d retired message(s)",len(pending))
        return True
    live["retiring_message_ids"]=pending
    logger.warning("Live pair cleanup deferred for %d message(s): %s",len(pending),text(res.get("description")))
    return False
...
def live_pair(state: dict, now: datetime) -> tuple[dict, dict, list[dict], list[dict]]:
    ...
    next_events,_=collect_events(ai, tomorrow, "next", allow_exa_fallback=True)
    past_events,_=collect_events(ai, yesterday, "past", allow_exa_fallback=True)
    next_filtered=filter_state(next_events,"next",tomorrow)
    past_filtered=filter_state(past_events,"past",yesterday)
    next_filtered=[e for e in next_filtered if isinstance(e.get("start"),datetime) and to_bd(e["start"]).date()==tomorrow]
    past_filtered=[e for e in past_filtered if isinstance(e.get("start"),datetime) and to_bd(e["start"]).date()==yesterday and e.get("state")=="final"]
    next_major=major_live_events(next_filtered,"next",tomorrow,limit=10)
    past_major=major_live_events(past_filtered,"past",yesterday,limit=10)
    if len(next_major) < 10 and next_filtered:
        fallback_next=major_live_events(next_filtered,"next",tomorrow,limit=10,min_score=0)
        seen={x.get("event_id") for x in next_major}
        next_major.extend(x for x in fallback_next if x.get("event_id") not in seen)
        next_major=next_major[:10]
    if len(past_major) < 10 and past_filtered:
        fallback_past=major_live_events(past_filtered,"past",yesterday,limit=10,min_score=0)
        seen={x.get("event_id") for x in past_major}
        past_major.extend(x for x in fallback_past if x.get("event_id") not in seen)
        past_major=past_major[:10]
    if not next_major:
        logger.warning("No valid next-day games found; schedule will use an explicit empty-state row")
    if not past_major:
        logger.warning("No valid previous-day results found; results will use an explicit empty-state row")
...
def record_coverage(coverage: dict, story: dict, run_id: str, message_id: int | None) -> None:
    rec={
        ...
        "source_urls": [canonical_url(u) for u in (story.get("research_sources") or story.get("urls",[])) if text(u)],
...
PROMPT_FALLBACKS = {
    "exa_sector_discovery_v1.txt": (
        "THE SPORTS NEWSROOM V1: NORMAL WEB SEARCH DISCOVERY\n\n"
        ...
        "The primary search provider is normal web search. The caller supplies a sector-specific intent. Exa is only a secondary fallback."
    ),
    ...
    "cerebras_editorial_v1.txt": (
        ...
        "Transform one verified evergreen Sports & Games research item into a clear, human-sounding Telegram post. "
        ...
        "Write ONE short paragraph of 40-60 words that explains the main fact clearly and naturally. Start with the subject and the most useful fact, then add the strongest supporting detail and brief context or significance when supported. Prefer 2-4 natural sentences over one long sentence. "
        "Use plain, natural sentences, active voice and concrete wording, as a knowledgeable sports journalist would. Avoid hype, clichés, marketing language, filler, vague praise, repetitive sentence patterns and generic AI-style phrases. "
        "Do not use an em dash or en dash. Use commas, periods or parentheses instead. Do not use lists or semicolon-heavy writing. Do not restate the same point twice. "
        "Headline: 6-14 words, specific and non-clickbait. Return exactly 2 relevant, topic-specific hashtags. Do not use current-news framing. "
...
#     Normal web search -> local candidate reservoir -> Cerebras ranking ->
...
NORMAL_SEARCH_RESULTS = max(3, _env_int("NORMAL_SEARCH_RESULTS", 6))
NORMAL_SEARCH_FALLBACK_MIN = max(1, _env_int("NORMAL_SEARCH_FALLBACK_MIN", 3))
NORMAL_SEARCH_TIMEOUT = max(8, _env_int("NORMAL_SEARCH_TIMEOUT", 15))
V1_CONTENTS_CHARS = _env_int("V1_CONTENTS_CHARS", 9000)
V1_RANK_MAX_CANDIDATES = 20
V1_RANK_MAX_TOKENS = max(512, min(900, _env_int("V1_RANK_MAX_TOKENS", 768)))
V1_EXCLUDE_DOMAINS = [
...
def v1_state(state: dict) -> dict:
    ...
    root["live"].setdefault("schedule", {"message_id": None, "target_date": ""})
    root["live"].setdefault("results", {"message_id": None, "target_date": ""})
    root["live"].setdefault("retiring_message_ids", [])
    ...
    if isinstance(old, dict):
        ...
        old_live=old.get("live") if isinstance(old.get("live"),dict) else {}
        root["live"]["retiring_message_ids"]=_unique_message_ids(root["live"].get("retiring_message_ids", []) + [
            old_live.get("schedule",{}).get("message_id"), old_live.get("results",{}).get("message_id")
        ])
...
def _v1_search_system(sector: str, target: date, used_sectors: set[str]) -> str:
    ...
    base = _v1_prompt("exa_sector_discovery_v1.txt", "Retrieve evergreen Sports & Games source pages using ordinary web search; do not write an article.")
    return (base + f"\n\nTARGET SECTOR: {sector}\nCALENDAR DATE: {target.isoformat()}\n"
            + f"PREVIOUSLY USED SECTORS TODAY: {used}\n"
            + "Return ordinary web search results. Do not return a synthetic publication object.")


class _SearchHTMLParser(HTMLParser):
    """Small dependency-free parser for normal search-engine result pages."""
    def __init__(self) -> None:
        super().__init__(convert_charrefs=True)
        self.results: list[dict] = []
        self._bing_item: dict | None = None
        self._bing_title_active = False
        self._bing_snippet_active = False
        self._buffer: list[str] = []
        self._snippet_buffer: list[str] = []
        self._ddg_title: dict | None = None
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == "li" and "b_algo" in classes:
            self._bing_item={"url":"","title":"","text":""}
            return
        if self._bing_item is not None:
            if tag == "a":
                href=next((v for k,v in attrs if k == "href"), "")
                if href and not self._bing_item.get("url"):
                    self._bing_item["url"]=href
                    self._bing_title_active=True
                    self._buffer=[]
                    return
            if "b_caption" in classes or (self._bing_snippet_active is False and self._bing_item.get("title") and tag == "p"):
                self._bing_snippet_active=True
                self._snippet_buffer=[]
                return
        if tag == "a" and "result__a" in classes:
            href=next((v for k,v in attrs if k == "href"), "")
            self._ddg_title={"url":href,"title":"","text":""}
            self._buffer=[]
            return
        if self._ddg_title is not None and tag in {"a","div","span"} and "result__snippet" in classes:
            self._ddg_title["text"]=""
            self._snippet_buffer=[]

    def handle_data(self, data):
        if self._bing_snippet_active:
            self._snippet_buffer.append(data)
        elif self._bing_title_active:
            self._buffer.append(data)
        elif self._ddg_title is not None:
            self._buffer.append(data)

    def handle_endtag(self, tag):
        if self._bing_title_active and tag == "a" and self._bing_item is not None:
            self._bing_item["title"]=" ".join(self._buffer).strip()
            self._bing_title_active=False
            self._buffer=[]
        if self._bing_snippet_active and tag in {"p","div"} and self._bing_item is not None:
            self._bing_item["text"]=" ".join(self._snippet_buffer).strip()
            self._bing_snippet_active=False
            self._snippet_buffer=[]
        if tag == "li" and self._bing_item is not None:
            if self._bing_item.get("url") and self._bing_item.get("title"):
                self.results.append(self._bing_item)
            self._bing_item=None
        if tag == "a" and self._ddg_title is not None:
            self._ddg_title["title"]=" ".join(self._buffer).strip()
            if self._ddg_title.get("url") and self._ddg_title.get("title"):
                self.results.append(self._ddg_title)
            self._ddg_title=None
            self._buffer=[]


def _search_redirect_url(raw: str) -> str:
    u=text(raw)
    if not u: return ""
    ...
    try:
        ...
        for key in ("uddg", "url", "u"):
            if qs.get(key):
                return v1_network_url(qs[key])
    except Exception:
        pass
    return v1_network_url(u)


def _normal_search_parse(raw_html: str, engine: str, limit: int) -> list[dict]:
    parser=_SearchHTMLParser()
    try:
        parser.feed(raw_html or "")
    except Exception as exc:
        logger.warning("Normal search HTML parse failed engine=%s: %s", engine, redact(str(exc)))
        return []
    if engine == "duckduckgo":
        snippets=re.findall(r'class=["\'][^"\']*result__snippet[^"\']*["\'][^>]*>(.*?)</', raw_html or "", flags=re.I|re.S)
        snippets=[re.sub(r"<[^>]+>"," ",html.unescape(x)) for x in snippets]
        snippets=[re.sub(r"\s+"," ",x).strip() for x in snippets]
        for i,item in enumerate(parser.results):
            if not item.get("text") and i < len(snippets): item["text"]=snippets[i]
    out=[]; seen=set()
    for item in parser.results:
        url=_search_redirect_url(text(item.get("url")))
        title=re.sub(r"\s+", " ", text(item.get("title"))).strip()
        snippet=re.sub(r"\s+", " ", text(item.get("text"))).strip()
        if not url or not title or not re.match(r"^https?://",url,re.I): continue
        cu=canonical_url(url)
        if not cu or cu in seen: continue
        seen.add(cu)
        if exclusion_reason(f"{title} {snippet} {url}"): continue
        if V1_CURRENT_TITLE_RX.search(title): continue
        out.append({"url":url,"title":title,"highlights":[snippet] if snippet else [],"text":snippet[:3500],
                    "source":source_label(url),"grade":grade_of(url),"image":"","search_provider":engine})
        if len(out)>=limit: break
    return out


def _normal_search_engine(query: str, engine: str, limit: int) -> list[dict]:
    if engine == "duckduckgo":
        url="https://html.duckduckgo.com/html/"
        params={"q":query,"kl":"us-en","kp":"-2"}
    elif engine == "bing":
        url="https://www.bing.com/search"
        params={"q":query,"count":str(limit),"setlang":"en-US"}
    else:
        return []
    r=http(f"normal-search:{engine}", "GET", url, params=params,
           headers={"Accept":"text/html,application/xhtml+xml"}, timeout=NORMAL_SEARCH_TIMEOUT,
           retries=0, want_json=False)
    if not r.ok or not r.text:
        logger.warning("Normal search failed engine=%s status=%s error=%s", engine, r.status, redact(r.error)[:180])
        return []
    return _normal_search_parse(r.text, engine, limit)


def normal_web_search(query: str, *, num: int=6, domains: list[str] | None=None) -> list[dict]:
    """Primary no-API-key web discovery. Exa is deliberately outside this function."""
    q=text(query)
    if domains:
        site_clause=" OR ".join(f"site:{d}" for d in domains[:8])
        q=f"({q}) ({site_clause})"
    rows=_normal_search_engine(q, "duckduckgo", max(1,int(num)))
    if len(rows) < max(2, min(int(num), NORMAL_SEARCH_FALLBACK_MIN)):
        extra=_normal_search_engine(q, "bing", max(1,int(num)))
        existing={canonical_url(x.get("url")) for x in rows}
        for row in extra:
            if canonical_url(row.get("url")) not in existing:
                rows.append(row); existing.add(canonical_url(row.get("url")))
            if len(rows)>=num: break
    return rows[:num]


def _candidate_from_search_result(sector: str, target: date, item: dict, provider: str) -> dict:
    url=v1_network_url(text(item.get("url")))
    title=re.sub(r"\s+", " ", text(item.get("title"))).strip()
    highlights=[re.sub(r"\s+", " ", text(h)) for h in (item.get("highlights") or []) if text(h)]
    excerpt=" ".join(highlights)[:3500] or re.sub(r"\s+", " ", text(item.get("text")))[:3500]
    return {
        "sector":sector,"normalized_subject":title,"central_knowledge_unit":title,
        "central_claim":excerpt[:700] or title,"research_focus":v1_sector_query(sector,target),"topic":title,
        "sport_or_game":"","source_url":url,"source":source_label(url,text(item.get("author"))),
        "grade":grade_of(url),"highlights":highlights[:6],"text":excerpt,"image":"","published":parse_dt(item.get("published")),
        "search_provider":provider,"favicon":"","search_request_id":"",
    }


def normal_search_sector(sector: str, target: date, used_sectors: set[str], *, num: int | None=None) -> list[dict]:
    query=v1_sector_query(sector,target)
    rows=normal_web_search(query,num=max(1,int(num or NORMAL_SEARCH_RESULTS)))
    out=[]
    for item in rows:
        c=_candidate_from_search_result(sector,target,item,text(item.get("search_provider")) or "normal")
        title=text(c.get("normalized_subject")); excerpt=text(c.get("text"))
        alltext=f"{title} {excerpt} {c.get('source_url','')}"
        reason=evergreen_scope_reason(c)
        if reason:
            reject("v1_scope_filter", f"{sector}: {title[:120]} ({reason})")
            continue
        if V1_CURRENT_TITLE_RX.search(title): continue
        out.append(c)
    return out
...
def _search_with_exa_fallback_sector(sector: str, target: date, used_sectors: set[str], *, num: int | None=None, hard: bool=False) -> list[dict]:
    rows=normal_search_sector(sector,target,used_sectors,num=num)
    if len(rows) >= NORMAL_SEARCH_FALLBACK_MIN:
        logger.info("V1 normal search primary sector=%s results=%d; Exa not used",sector,len(rows))
        return rows
    logger.warning("V1 normal search sparse sector=%s results=%d; using Exa fallback",sector,len(rows))
    exa=v1_exa_search_sector(sector,target,used_sectors,mode="deep" if hard else "auto",num=num)
    return rows + exa
...
def v1_exa_search_sector(sector: str, target: date, used_sectors: set[str], *, mode: str = "auto", num: int | None = None) -> list[dict]:
    ...
    for item in r.data.get("results", []) or []:
        ...
        candidate_probe={"sector":sector,"source_url":url,"normalized_subject":title,"text":excerpt,"highlights":highlights,"grade":grade_of(url)}
        scope_reason=evergreen_scope_reason(candidate_probe)
        if scope_reason:
            reject("v1_scope_filter", f"{sector}: {title[:120]} ({scope_reason})")
            continue
        published = parse_dt(item.get("publishedDate") or item.get("published_date"))
...
def v1_discover_all_sectors(target: date, used_sectors: set[str], *, recovery_sectors: set[str] | None = None) -> list[dict]:
    ...
    raw: list[dict] = []
    # Normal web search is primary and uses no paid search API. Exa is only invoked when a sector
    # returns too few usable results. Keep discovery sequential to avoid search-engine bursts.
    for sector in sectors:
        rows = _search_with_exa_fallback_sector(
            sector, target, used_sectors,
            num=NORMAL_SEARCH_RESULTS,
            hard=bool(recovery_sectors and sector in V1_HARD_SECTORS),
        )
        raw.extend(rows)
...
def v1_build_reservoir(raw: list[dict], coverage: dict, used_sectors: set[str]) -> tuple[list[dict], list[str]]:
    ...
    for c in ordered:
        scope_reason=evergreen_scope_reason(c)
        if scope_reason:
            reasons.append(f"scope:{scope_reason}")
            reject("v1_scope_filter", f"{text(c.get('sector'))}: {text(c.get('normalized_subject'))[:120]} ({scope_reason})")
            continue
        ...
        c["sources"] = _source_records([{"name":c.get("source"),"url":network_url,"type":f"{text(c.get('search_provider')) or 'Web search'} result","evidence_note":" ".join(c.get("highlights") or [])[:500]}])
...
def _extract_visible_page_text(raw_html: str) -> str:
    if not raw_html:
        return ""
    cleaned=re.sub(r"<script\b[^>]*>.*?</script>|<style\b[^>]*>.*?</style>|<noscript\b[^>]*>.*?</noscript>", " ", raw_html, flags=re.I|re.S)
    article=re.search(r"<article\b[^>]*>(.*?)</article>", cleaned, flags=re.I|re.S)
    body=article.group(1) if article else cleaned
    body=re.sub(r"<!--.*?-->"," ",body,flags=re.S)
    body=re.sub(r"<[^>]+>"," ",body)
    return re.sub(r"\s+"," ",html.unescape(body)).strip()[:12000]


def fetch_normal_source_evidence(url: str) -> dict | None:
    nu=v1_network_url(url)
    if not nu or requests is None:
        return None
    try:
        r=session().get(nu, headers={**HEADERS,"Accept":"text/html,application/xhtml+xml"}, timeout=HTTP_TIMEOUT, allow_redirects=True, stream=True)
        if int(getattr(r,"status_code",0) or 0)!=200:
            return None
        ctype=text(getattr(r,"headers",{}).get("Content-Type")).lower()
        ...
        for chunk in r.iter_content(chunk_size=64*1024):
            ...
            raw+=chunk
        page_text=_extract_visible_page_text(raw.decode("utf-8",errors="ignore"))
        if len(re.findall(r"\b\w+\b",page_text))<120:
            return None
        title_match=re.search(r"<title\b[^>]*>(.*?)</title>",raw.decode("utf-8",errors="ignore"),flags=re.I|re.S)
        title=re.sub(r"\s+"," ",html.unescape(re.sub(r"<[^>]+>"," ",title_match.group(1) if title_match else ""))).strip()
        return {"url":text(getattr(r,"url","") or nu),"title":title,"author":"","published":None,"text":page_text,"highlights":[page_text[:1200]],"summary":"","status":"ok"}
    except Exception as exc:
        logger.info("Normal source evidence unavailable %s: %s",domain_of(nu),redact(str(exc))[:140])
        return None
...
def v1_secondary_search(candidate: dict, target: date) -> list[dict]:
    ...
    rows=normal_web_search(q,num=5)
    if len(rows) < NORMAL_SEARCH_FALLBACK_MIN:
        logger.warning("V1 secondary normal search sparse; using Exa fallback")
...
def v1_verify_selected(selected: list[dict], target: date) -> tuple[list[dict], str]:
    ...
    for original in selected:
        c=json.loads(json.dumps(original, default=str))
        c["sources"]=_source_records(c.get("sources"))
        urls=[s["url"] for s in c["sources"]]
        if not urls:
            reject("v1_no_source",text(c.get("normalized_subject"))); continue

        pages={}
        for u in urls[:V1_CONTENTS_URLS_PER_STORY]:
            page=fetch_normal_source_evidence(u)
            if page:
                pages[canonical_url(u)]=page

        evidence=v1_evidence_text(c,pages)
        # Paid Exa Contents is a fallback enhancer, never the default evidence path.
        if len(evidence.strip())<500 and EXA_API_KEY:
            extra_pages,_=v1_contents_for_urls(urls[:V1_CONTENTS_URLS_PER_STORY])
            pages.update(extra_pages)
            evidence=v1_evidence_text(c,pages)

        # For strong historical/first-last claims, try to add an independent normal-search source.
        if len(evidence.strip())<500 or v1_hard_case_needed(c,pages):
            extra=v1_secondary_search(c,target)
            for row in extra:
                u=v1_network_url(text(row.get("url")))
                if not u or canonical_url(u) in {canonical_url(x) for x in urls}:
                    continue
                c["sources"]=_source_records(c["sources"]+[row])
                urls=[s["url"] for s in c["sources"]]
                page=fetch_normal_source_evidence(u)
                if page:
                    pages[canonical_url(u)]=page
                if len(urls)>=V1_CONTENTS_URLS_PER_STORY:
                    break
            evidence=v1_evidence_text(c,pages)
            if len(evidence.strip())<500 and EXA_API_KEY:
                extra_pages,_=v1_contents_for_urls(urls[:V1_CONTENTS_URLS_PER_STORY])
        ...
        # One verified, readable source page is the minimum. Never bypass this gate.
        available=[u for u in urls if canonical_url(u) in pages and len(text(pages.get(canonical_url(u),{}).get("text")))>=300]
        if not available or len(evidence.strip())<500:
            reject("v1_evidence_insufficient",text(c.get("normalized_subject"))); continue

        if v1_hard_case_needed(c,pages) and agent_cases<V1_AGENT_MAX_CASES and EXA_API_KEY:
            ...
            if agent:
                ...
                if text(agent.get("evidence_summary")):
                    evidence=(evidence+"\n\nAGENT VERIFICATION:\n"+text(agent.get("evidence_summary"))[:5000]).strip()

        c["sources"]=_source_records(c.get("sources"))
        c["verified_pages"]={canonical_url(u):pages[canonical_url(u)] for u in urls if canonical_url(u) in pages}
...
def v1_normalize_story(story: dict) -> dict:
    ...
    # Selected external imagery is already fetched and verified before publication. Preserve only
    # metadata here; the local file path remains ephemeral and is passed directly to Telegram.
    img=out.get("image") if isinstance(out.get("image"),dict) else {}
    if text(img.get("url")) and text(out.get("image_status")) == "verified_external":
        out["image"]={k:img[k] for k in ("url","source_page_url","source_name","provider","bytes","width","height","score") if k in img}
    else:
        out["image"]={}
    src=_source_records(out.get("sources"))
    out["sources"]=[(x["name"],x["url"]) for x in src]
    out["urls"]=[x["url"] for x in src]
    ...
    out["image_status"]=text(out.get("image_status")) or "pending"
    return out
...
def v1_validate_editorial(candidate: dict, editorial: dict) -> tuple[bool, str, dict]:
    ...
    if "—" in headline or "—" in body: return False,"em_dash_style",{}
    if "–" in headline or "–" in body: return False,"en_dash_style",{}
    ...
    if len(hashtags) > 2: return False,"hashtag_count",{}
...
def v1_editorialize(ai: "AIClient", candidate: dict, evidence: str) -> dict | None:
    ...
    return ai.json("v1_editorial_post",system,user,EDITORIAL_SCHEMA,max_tokens=max(384,min(700,_env_int("V1_EDITORIAL_MAX_TOKENS",512))),temperature=0.35)


def v1_publish_evergreen(story: dict, media_path: str | None = None) -> dict:
    """Publish one evergreen post using the already-selected media; external-photo failure retries with a card."""
    story=v1_normalize_story(story)
    rendered=render_evergreen_post(story)
    chosen_path=media_path if media_path and Path(media_path).is_file() else None
    if not chosen_path:
        chosen_path,_=prepare_evergreen_media(story)
    if not chosen_path:
        return {"ok":False,"description":"V1 evergreen publication failed: no usable media and card generation failed"}
    try:
        payload={
            "chat_id":CHANNEL,
        ...
        result=tg_call("sendPhoto",payload,chosen_path)
        if result.get("ok"):
            return result

        # A verified external image can still fail Telegram-side decoding/transport. Retry once with
        # the deterministic branded card. Never re-run image discovery here.
        if text(story.get("image_status")) == "verified_external":
            logger.warning("Verified story photo upload failed; retrying evergreen post with branded card: %s", redact(text(result.get("description")))[:180])
            card=make_card(story)
            if not card:
                return result
            story["image_status"]="card_fallback"
            try:
                return tg_call("sendPhoto",payload,card)
            finally:
                if Path(card).exists():
                    try: os.remove(card)
                    except OSError: pass
        ...
        if chosen_path and Path(chosen_path).exists():
            try: os.remove(chosen_path)
            except OSError: pass


def rotate_live_pair(state: dict, vstate: dict, run_id: str,
                     sched_meta: dict, res_meta: dict,
                     next_events: list[dict], past_events: list[dict],
                     *, dry: bool) -> dict:
    """Publish replacement live messages, then retire the previous pair.

    Previous message IDs are persisted before deletion. If cleanup fails, those IDs remain
    queued in `retiring_message_ids` and are retried at the start of the next run.
    """
    if not dry:
        cleanup_live_retirements(vstate)
    old_sched=vstate["live"]["schedule"].get("message_id")
    old_res=vstate["live"]["results"].get("message_id")
    old_sched_target=text(vstate["live"]["schedule"].get("target_date"))
    old_res_target=text(vstate["live"]["results"].get("target_date"))
    known=known_live_message_ids(state,vstate)
    # Retire historical/orphaned live messages, but keep the currently active pair until a
    # valid replacement exists. Stale active targets are retired even if that side is empty.
    next_target=text(sched_meta.get("target_date"))
    result_target=text(res_meta.get("target_date"))
    replaced=[mid for mid in known if mid not in {old_sched,old_res}]
    if old_sched and old_sched_target != next_target:
        replaced.append(old_sched)
    if old_res and old_res_target != result_target:
        replaced.append(old_res)
    replaced=_unique_message_ids(replaced)

    def publish_or_preview(kind: str, target: date, events: list[dict], old_id: int | None, old_target: str) -> dict:
        if dry:
            return {"ok":True,"skipped":False,"dry_run":True,"result":{"message_id":None}}
        if not events and old_id and old_target == target.isoformat():
            return {"ok":True,"skipped":True,"retained":True,"result":{"message_id":old_id}}
        return publish_with_idempotency(vstate,publication_id(run_id,kind,target.isoformat()),lambda:publish_live(kind,target,events))

    sched_target=date.fromisoformat(sched_meta["target_date"])
    res_target=date.fromisoformat(res_meta["target_date"])
    sr=publish_or_preview("live_schedule",sched_target,next_events,old_sched,old_sched_target)
    rr=publish_or_preview("live_results",res_target,past_events,old_res,old_res_target)
    if not sr.get("ok"):
        logger.warning("New V1 live schedule failed; retaining previous schedule")
    if not rr.get("ok"):
        logger.warning("New V1 live results failed; retaining previous results")
    if dry:
        return {"schedule":sr,"results":rr,"cleanup_ok":True}

    if sr.get("ok") and not sr.get("skipped"):
        new_sched=(sr.get("result") or {}).get("message_id")
        if new_sched:
            if old_sched and int(old_sched)!=int(new_sched): replaced.append(int(old_sched))
            replaced=_unique_message_ids(replaced)
            vstate["live"]["schedule"]={"message_id":new_sched,"target_date":sched_meta["target_date"],"updated_at":utc_iso(now_bd()),"event_ids":[x.get("event_id") for x in next_events]}
    elif not next_events:
        logger.info("Live schedule skipped: no major next-day events")

    if rr.get("ok") and not rr.get("skipped"):
        new_res=(rr.get("result") or {}).get("message_id")
        if new_res:
            if old_res and int(old_res)!=int(new_res): replaced.append(int(old_res))
            replaced=_unique_message_ids(replaced)
            vstate["live"]["results"]={"message_id":new_res,"target_date":res_meta["target_date"],"updated_at":utc_iso(now_bd()),"event_ids":[x.get("event_id") for x in past_events]}
    elif not past_events:
        logger.info("Live results skipped: no major previous-day results")

    pending=_unique_message_ids(vstate["live"].get("retiring_message_ids", []))
    vstate["live"]["retiring_message_ids"]=_unique_message_ids(pending+replaced)
    state["last_run_at"]=utc_iso(now_bd())
    save_state(state)
    cleanup_ok=cleanup_live_retirements(vstate,exclude_ids=[vstate["live"]["schedule"].get("message_id"),vstate["live"]["results"].get("message_id")])
    save_state(state)
    return {"schedule":sr,"results":rr,"cleanup_ok":cleanup_ok,"retired_ids":replaced}


def prune_state(state: dict) -> None:
    """Keep mutable runtime state bounded while preserving the longer-lived coverage index separately."""
    root=state.get(V1_STATE_KEY) if isinstance(state.get(V1_STATE_KEY),dict) else {}
    cutoff=now_bd()-timedelta(days=max(30,STATE_RETENTION_DAYS))
    cutoff_iso=utc_iso(cutoff)
    if isinstance(root.get("runs"),list):
        root["runs"]=[r for r in root["runs"] if isinstance(r,dict) and text(r.get("at"))>=cutoff_iso][-240:]
    if isinstance(root.get("daily"),dict):
        root["daily"]={k:v for k,v in root["daily"].items() if k >= cutoff.date().isoformat()}
    if isinstance(root.get("publication"),dict):
        root["publication"]={k:v for k,v in root["publication"].items() if not isinstance(v,dict) or text(v.get("posted_at") or v.get("started_at"))>=cutoff_iso}
    if isinstance(state.get("posts"),list):
        state["posts"]=[p for p in state["posts"] if not isinstance(p,dict) or text(p.get("posted_at"))>=cutoff_iso][-1000:]


def known_live_message_ids(state: dict, vstate: dict) -> list[int]:
    ids=[]
    live=vstate.get("live") or {}
    for key in ("schedule","results"):
        if isinstance(live.get(key),dict): ids.append(live[key].get("message_id"))
    for run in vstate.get("runs",[]) if isinstance(vstate.get("runs"),list) else []:
        if isinstance(run,dict): ids.extend([run.get("live_schedule"),run.get("live_results")])
    for p in state.get("posts",[]) if isinstance(state.get("posts"),list) else []:
        if isinstance(p,dict) and text(p.get("desk")) in {"next","past","live_schedule","live_results"}: ids.append(p.get("message_id"))
    return _unique_message_ids(ids)
...
def v1_run_once(mode: str = "live") -> int:
    ...
    for c in selected + ordered:
        sec=text(c.get("sector"))
        if sec in verification_sectors:
            continue
        verification_targets.append(c); verification_sectors.add(sec)
        if len(verification_targets)>=min(16,len(selected)+len(ordered)): break
    verified,status=v1_verify_selected(verification_targets,now.date()) if verification_targets else ([],"skipped")
    if status!="ok":
        logger.warning("V1 verification partial/unavailable: %s; unverified candidates are skipped",status)
    verified_by_id={text(c.get("candidate_id")):c for c in verified if text(c.get("candidate_id"))}
    stream=[]
    for c in selected + ordered:
        vc=verified_by_id.get(text(c.get("candidate_id")))
        if vc and text(vc.get("sector")) not in {text(x.get("sector")) for x in stream}:
            stream.append(vc)
    posted_sectors=set()
    for cand in stream:
        ...
        if not dry:
            existing_pub=vs.get("publication",{}).get(pubid)
            if isinstance(existing_pub,dict) and existing_pub.get("status")=="posted":
                logger.info("V1 evergreen publication already posted; skipping image research for %s",story.get("normalized_subject")); continue
        media_path,_=prepare_evergreen_media(story)
        res=publish_with_idempotency(vs,pubid,lambda st=story,mp=media_path,do=dry: v1_publish_evergreen(st,mp) if not do else {"ok":True,"dry_run":True,"result":{"message_id":None}})
        if dry and media_path:
        ...
        state.setdefault("posts",[]).append({
            ...
            "urls":[canonical_url(u) for u in story.get("urls",[])],"research_urls":[canonical_url(u) for u in (story.get("research_sources") or story.get("urls",[])) if text(u)],"message_id":mid,"posted_at":utc_iso(now_bd()),"run_id":run_id,
    ...
    live_result=rotate_live_pair(state,vs,run_id,sched_meta,res_meta,next_events,past_events,dry=dry)
    if not dry and not live_result.get("cleanup_ok"):
        logger.warning("V1 live pair published, but retired messages remain queued for deletion")
...
def v1_architecture_tests() -> int:
    ...
    ck(source_public_name("https://www.bbc.co.uk/sport/example")=="BBC Sport", "source public label")
    ck(source_public_name("https://example-sports-site.com/story")=="Example Sports Site", "unknown source does not expose raw domain")
    ck(tg_sanitize(rendered_html) == rendered_html, "evergreen HTML is Telegram-safe")
    public_lines=rendered_html.splitlines()
    hashtag_lines=[ln for ln in public_lines if ln.strip().startswith("#")]
    ck(any(line == "<b><a href=\"https://t.me/TheSportsNewsroom\">Sports News</a></b> #Football #History" for line in public_lines), "channel and hashtags share one line")
    ...
    ck(f'<b><a href="{esc_attr(CHANNEL_URL)}">{esc(CHANNEL_DISPLAY_NAME)}</a></b>' in rendered_html, "channel name is bold and linked")
    ck(CHANNEL_HANDLE == "@TheSportsNewsroom", "channel handle derived correctly")
    ...
    sample_bing='<a class="iusc" m="{&quot;murl&quot;:&quot;https://img.example/photo.jpg&quot;,&quot;purl&quot;:&quot;https://example.com/story&quot;,&quot;t&quot;:&quot;Example football photo&quot;}"></a>'
    parsed_images=_parse_bing_image_results(sample_bing,1)
    ck(len(parsed_images)==1 and parsed_images[0]["image_url"]=="https://img.example/photo.jpg" and parsed_images[0]["page_url"]=="https://example.com/story", "normal image search keeps original image URL")
    ck(IMAGE_SEARCH_RESULTS >= 4 and MAX_IMAGE_BYTES <= 10_000_000, "image verification bounds")
    ck(IMAGE_ALLOWED_MIME == {"image/jpeg","image/png","image/webp"}, "supported photo MIME contract")
    ...
    normalized=v1_normalize_story({"headline":"Why This Sports Measurement Still Matters","body":"This is a verified evergreen story with enough detail to satisfy the editorial body length requirements in a real post.","sources":[("Source","https://example.com")],"tags":["#SportsFacts"],"image":{}})
    ck(normalized["image"] == {} and normalized["image_status"] == "pending", "image metadata normalization")
    source_story=build_evergreen({"sector":"Sport History","normalized_subject":"Test subject","sources":[{"name":"Weak Source","url":"https://example.com/weak","grade":"C","evidence_note":"short"},{"name":"Primary Source","url":"https://fifa.com/example","grade":"A","evidence_note":"documented authoritative evidence"}],"central_claim":"Test claim"},{"headline":"A Sporting Story With A Clear History","body":ed["body"],"hashtags":["#SportHistory","#GameHistory"]})
    ck(len(source_story["sources"]) == 1 and source_story["sources"][0][0] == "FIFA", "public evergreen story keeps one best source")
    ck(len(source_story.get("research_sources",[])) == 2, "research source list preserved internally")
    # Final evergreen public format: one source, two hashtags, channel identity, fixed ordering.
    render_sample={"headline":"The Origins of a Sporting Tradition","body":"This verified evergreen story explains how a documented sporting tradition developed and why its surviving rules still matter. The evidence connects its early history with later organized play, giving readers a compact view of how a game moved from local practice into a broader sporting culture over time.","sources":[("Source A","https://example.com/a"),("Source B","https://example.com/b")],"tags":["#SportHistory","#AncientGames","#Extra"]}
    research_story={**render_sample,"research":{"sources":[{"name":"Weak Source","url":"https://example.com/weak","grade":"C","evidence_note":"short"},{"name":"Primary Source","url":"https://fifa.com/example","grade":"A","evidence_note":"authoritative evidence"}]}}
    rendered2=render_evergreen_post(research_story)
    public_html=rendered2["html"]
    ck(rendered2["parse_mode"] == "HTML", "evergreen parse mode")
    ck(public_html.count("<a href=") == 2, "evergreen has exactly channel+source links")
    ck(public_html.count("Source:") == 1, "evergreen has one source line")
    ck(public_html.splitlines()[-1].startswith("Source: <a href="), "source is final line")
    channel_tag_line=next((ln for ln in public_html.splitlines() if "<b><a href=" in ln and "Sports News" in ln), "")
    ck(len(re.findall(r"#[A-Za-z0-9]+", channel_tag_line)) == 2, "evergreen has exactly two hashtags")
    ck("https://example.com/b" not in re.sub(r'href="[^"]+"', '', public_html), "non-public source URL hidden")
    ck(IMAGE_ALLOWED_MIME == {"image/jpeg","image/png","image/webp"}, "image MIME contract supports webp normalization")
    ck(IMAGE_MIN_ASPECT <= 0.75 and IMAGE_MAX_ASPECT >= 1.9, "image geometry guard")
    fd, placeholder_path=tempfile.mkstemp(prefix="sn_blank_",suffix=".jpg"); os.close(fd)
    try:
        if Image is not None:
            Image.new("RGB",(640,360),(255,255,255)).save(placeholder_path,"JPEG")
            ck(_image_is_probable_placeholder(placeholder_path), "white placeholder photo rejected")
            Image.new("RGB",(640,360),(70,100,160)).save(placeholder_path,"JPEG")
            ck(not _image_is_probable_placeholder(placeholder_path), "normal photo raster allowed")
    finally:
        Path(placeholder_path).unlink(missing_ok=True)
    ck(len(live_rows([make_event(sport="Football",league="Test",home="A",away="B",name="A vs B",start=datetime(2026,9,24,12,tzinfo=timezone.utc),state="scheduled")],"next")[0]) == 3, "live table has SPORT GAME TIME columns")
    ck("Asia/Dhaka" not in json.dumps(live_rich("next",date(2026,9,24),[]),ensure_ascii=False), "live public message hides internal timezone")
    # Live-date contracts.
    probe_now=datetime(2026,9,23,18,0,tzinfo=BD_TZ)
    old_collect=globals().get("collect_events")
    try:
        globals()["collect_events"] = lambda *args, **kwargs: ([], "ok")
        n_meta,y_meta,_,_=live_pair({},probe_now)
        ck(y_meta["target_date"] == "2026-09-22" and n_meta["target_date"] == "2026-09-24", "live target dates are previous/next day")
    finally:
        globals()["collect_events"] = old_collect
    final_ev=make_event(sport="Football",league="Test",home="A",away="B",name="A vs B",start=datetime(2026,9,22,12,tzinfo=timezone.utc),state="final",home_score=2,away_score=1)
    blank_ev=make_event(sport="Football",league="Test",home="C",away="D",name="C vs D",start=datetime(2026,9,22,13,tzinfo=timezone.utc),state="final",home_score="",away_score="")
    rr=live_rows([final_ev,blank_ev],"past")
    ck(len(rr) == 2 and rr[1][2]["text"] == "2–1", "results table contains only real results")

    # Telegram-side image failure must transparently fall back to the branded card.
    fd, test_media=tempfile.mkstemp(prefix="sn_test_photo_", suffix=".jpg"); os.close(fd)
    try:
        if Image is not None:
            Image.new("RGB", (320,180), (10,20,30)).save(test_media, "JPEG")
            old_prepare=globals().get("prepare_evergreen_media"); old_tg=globals().get("tg_call")
            calls=[]
            def fake_prepare(story):
                story["image_status"]="verified_external"
                return test_media, {"mode":"external"}
            def fake_tg(method,data=None,file_path="",file_field="photo"):
                calls.append((method,file_path))
                if len(calls)==1:
                    return {"ok":False,"description":"telegram rejected photo"}
                return {"ok":True,"result":{"message_id":123}}
            try:
                globals()["prepare_evergreen_media"]=fake_prepare; globals()["tg_call"]=fake_tg
                probe_story={"headline":render_sample["headline"],"body":render_sample["body"],"sources":[("Source A","https://example.com/a")],"tags":["#SportHistory","#AncientGames"],"image_status":"pending"}
                result=v1_publish_evergreen(probe_story,None)
                ck(result.get("ok") and len(calls)==2 and calls[0][0]=="sendPhoto" and calls[1][0]=="sendPhoto", "external photo upload failure falls back to branded card")
                # Prepared card must be reused, not trigger a second media search.
                original_prepare=globals()["prepare_evergreen_media"]
                prepared_calls=[]
                globals()["prepare_evergreen_media"] = lambda story: (prepared_calls.append(1) or (test_media,{"mode":"card"}))
                fd2, test_media2=tempfile.mkstemp(prefix="sn_test_card_", suffix=".jpg"); os.close(fd2)
                if Image is not None:
                    Image.new("RGB", (320,180), (20,30,40)).save(test_media2, "JPEG")
                try:
                    prepared_story={**probe_story,"image_status":"card_fallback"}
                    res2=v1_publish_evergreen(prepared_story,test_media2)
                    ck(res2.get("ok") and len(prepared_calls)==0, "preselected fallback media is reused without rediscovery")
                finally:
                    globals()["prepare_evergreen_media"] = original_prepare
                    try: os.remove(test_media2)
                    except OSError: pass
            finally:
                globals()["prepare_evergreen_media"]=old_prepare; globals()["tg_call"]=old_tg
    finally:
        try: os.remove(test_media)
        except OSError: pass
...
def _run_diagnose() -> int:
    ...
    print("EXA FALLBACK:", "configured" if exa_ok else "not configured (normal search remains primary)")
    ai=AIClient()
...
def v1_validate_config(require_secrets: bool=True) -> int:
    ...
    if require_secrets:
        miss=[k for k,v in (("CEREBRAS_API_KEY",CEREBRAS_API_KEY),("TELEGRAM_BOT_TOKEN",TELEGRAM_BOT_TOKEN)) if not v]
        if miss:
    ...
    print(f"CONFIG: OK (The Sports Newsroom V1, {len(V1_SECTORS)} sectors, normal web search primary with Exa fallback)")
    return 0
