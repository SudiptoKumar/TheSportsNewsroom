#!/usr/bin/env python3
# The Sports Newsroom v4 - evergreen editorial + live sports newsroom for @TheSportsNewsroom.
#
# Design (see README.md):
#   Exa discovers -> Coverage remembers -> Cerebras ranks/edits -> code verifies -> Telegram publishes.
...
def v4_exa_validate_package(package: dict, required_sectors: list[str], target: date | None = None) -> tuple[bool, list[str], list[dict]]:
    errors: list[str] = []
    if not isinstance(package, dict):
        return False, ["package_not_object"], []
    if package.get("daily_complete") is not True:
        errors.append("daily_complete_false")
    if target is not None and text(package.get("calendar_date")) and text(package.get("calendar_date")) != target.isoformat():
    ...
    if required_sectors and not V4_SECTOR_SET.issuperset(required_sectors):
        errors.append("invalid_required_sector_set")
    for i, c in enumerate(candidates, 1):
        if not c["normalized_subject"] or not c["central_knowledge_unit"] or not c["central_claim"]:
            errors.append(f"missing_core:{i}")
        if V4_CURRENT_NEWS_RX.search(" ".join([c["headline"], c["main_story"], c["central_claim"]])):
            errors.append(f"current_news:{i}")
        if len(c["sources"]) == 0:
            errors.append(f"no_source:{i}")
        for src in c["sources"]:
            if not re.match(r"^https?://", src["url"], re.I):
                errors.append(f"bad_source_url:{i}")
        img = c.get("image", {}) or {}
        if isinstance(img, dict) and text(img.get("image_url")) and not re.match(r"^https?://", text(img.get("image_url")), re.I):
            errors.append(f"bad_image_url:{i}")
...
def v4_exa_research(ai: "AIClient", state: dict, coverage: dict, target: date, run_id: str, required_sectors: list[str], recovery_note: str = "") -> tuple[list[dict], list[dict], str]:
    if not EXA_API_KEY:
    ...
    exclusions = v4_coverage_context(coverage, target, set(required_sectors))
    ...
    query = (
        f"Research evergreen Sports & Games content for {target.isoformat()} {target.year}. "
        "Find one strong, independently supportable subject for every required sector. "
        "Search globally, prioritize authoritative historical and specialist sources, and exclude current sports news."
    )
    if recovery_note:
        query += " Recovery pass: avoid every previously rejected, already-covered, or near-duplicate knowledge unit and search for materially different subjects. " + recovery_note[:1200]
    additional = [
        ...
        "athlete historical figures births deaths founders sports institutions",
    ...
    if isinstance(content, str):
        try:
            package = extract_json(content)
        except Exception as exc:
            return [], r.data.get("results") or [], f"Exa output JSON invalid: {exc}"
    elif isinstance(content, dict):
        package = content
    else:
        # Some clients/versions may place structured output directly under output.content-like fields.
        package = r.data.get("content") if isinstance(r.data.get("content"), dict) else None
    if not isinstance(package, dict):
        return [], r.data.get("results") or [], "Exa did not return structured output.content"
    ok, errors, posts = v4_exa_validate_package(package, required_sectors, target)
    if not ok:
        REPORT.errors.append("Exa package rejected: " + ", ".join(errors[:12]))
        return [], r.data.get("results") or [], "invalid Exa package: " + ", ".join(errors[:10])
...
def v4_result_evidence(results: list[dict], urls: list[str]) -> str:
...
def v4_editorialize(ai: "AIClient", candidate: dict, evidence: str, image_candidates: list[dict]) -> dict | None:
    if not ai.available or ai.fatal:
        return None
    image_block = "\n".join(f"[{i}] {x.get('source_name','')} | {x.get('source_page_url','')} | {x.get('url','')}" for i, x in enumerate(image_candidates)) or "none"
    user = json.dumps({
        "research": candidate,
...
def evidence_text_for_validation(candidate: dict) -> str:
    ...
    bits=[candidate.get("main_story",""), candidate.get("central_claim",""), candidate.get("historical_date",""), candidate.get("historical_year","")]
...
def v4_run_once() -> int:
    ...
    required_sectors=[s for s in V4_SECTORS if s not in used_sectors]
    if len(required_sectors) < V4_PUBLISH_COUNT:
        # More than 10 sectors may be absent after a partial first run. Fill the exact next ten unused sectors.
        required_sectors=required_sectors[:V4_PUBLISH_COUNT]
    research, exa_results, status=v4_exa_research(ai,state,coverage,now.date(),run_id,required_sectors)
    if not research:
    ...
    if len(filtered) < V4_RESEARCH_COUNT:
        recovery_note = "Required unused sectors for this run: " + ", ".join(required_sectors) + ". Rejected candidate subjects: " + "; ".join(reasons[:25])
        research2, exa_results2, status2 = v4_exa_research(ai,state,coverage,now.date(),run_id,required_sectors,recovery_note=recovery_note)
        if research2:
    ...
    for c in ordered:
        if c.get("sector") in selected_sectors: continue
    ...
    if len(selected)<V4_PUBLISH_COUNT:
        logger.error("Cerebras selection yielded %d/%d usable sectors",len(selected),V4_PUBLISH_COUNT); return 1
    ...
    for idx,cand in enumerate(candidate_stream):
        ...
        image_candidates=v4_image_candidates(cand,exa_results)
        valid_images=[x for x in image_candidates if v4_validate_image_candidate(x,do_network=not (V4_DRY_RUN or DRY_RUN))]
        editorial=v4_editorialize(ai,cand,v4_result_evidence(exa_results,[x.get("url") for x in cand.get("sources",[])]),valid_images)
...
def v4_self_test(fast: bool=False) -> int:
    # Run the original V3 112-check suite against its fake network, then run focused V4 architecture tests.
...
def v4_main(argv: list[str] | None=None) -> int:
    ...
    ap.add_argument("--self-test",action="store_true",help="run the legacy V3 regression suite plus V4 architecture tests")
...
# Public names used by operational tooling.
run_once = v4_run_once
self_test = v4_self_test
