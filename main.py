...
IMAGE_ALLOWED_MIME = {"image/jpeg", "image/png"}
...
def _image_relevance(story: dict, candidate: dict) -> float:
    ...
    hay=" ".join([text(candidate.get("title")),text(candidate.get("alt")),text(candidate.get("page_title")),text(candidate.get("page_url")),text(candidate.get("source_name"))])
    got=tokens(hay)
    if not wanted or not got:
        return 0.0
    overlap=len(wanted & got)/max(1,len(wanted))
    exact=0.0
    subject=normalize_text(story.get("normalized_subject"))
    hay_norm=normalize_text(hay)
    if subject and subject in hay_norm:
        exact=0.45
    ...
    return min(1.0, overlap*0.60 + exact + person_bonus)
...
def _extract_meta_image_candidates(page_url: str, raw_html: str) -> list[dict]:
    """Extract lead-image metadata from a known source page before image search fallback."""
    out=[]
    patterns = [
        (r'<meta[^>]+property=["\']og:image(?::secure_url)?["\'][^>]+content=["\']([^"\']+)["\']', "og:image"),
        (r'<meta[^>]+name=["\']twitter:image(?::src)?["\'][^>]+content=["\']([^"\']+)["\']', "twitter:image"),
        (r'<meta[^>]+itemprop=["\']image["\'][^>]+content=["\']([^"\']+)["\']', "itemprop:image"),
        (r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+property=["\']og:image(?::secure_url)?["\']', "og:image"),
        (r'<meta[^>]+content=["\']([^"\']+)["\'][^>]+name=["\']twitter:image(?::src)?["\']', "twitter:image"),
    ]
    for rx, meta_name in patterns:
        for match in re.findall(rx, raw_html or "", flags=re.I | re.S):
            u = v1_network_url(urljoin(page_url, html.unescape(text(match))))
            if u:
                out.append({"image_url": u, "page_url": page_url, "title": page_url, "alt": meta_name, "source_name": source_label(page_url), "provider": "source_page"})
    # Lightweight JSON-LD fallback. It intentionally only accepts an explicit absolute/relative image URL.
    for block in re.findall(r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>', raw_html or "", flags=re.I | re.S):
        for match in re.findall(r'\"image\"\s*:\s*(?:\"([^\"]+)\"|\[\s*\"([^\"]+)\")', html.unescape(block), flags=re.I | re.S):
            raw = text(next((x for x in match if x), ""))
            u = v1_network_url(urljoin(page_url, raw))
            if u:
                out.append({"image_url": u, "page_url": page_url, "title": page_url, "alt": "json-ld:image", "source_name": source_label(page_url), "provider": "source_page"})
    return out
...
def discover_evergreen_image_candidates(story: dict) -> list[dict]:
    """Build a ranked image queue: evidence-page lead images -> normal image search -> Exa fallback."""
    candidates=[]; seen=set()
    source_pages=[]
    for item in story.get("sources", []) if isinstance(story.get("sources"), list) else []:
        if isinstance(item, (list, tuple)) and len(item)>=2 and text(item[1]):
            source_pages.append(v1_network_url(text(item[1])))
        elif isinstance(item, dict) and text(item.get("url")):
            source_pages.append(v1_network_url(text(item.get("url"))))
    for page in source_pages[:2]:
        for c in _fetch_source_page_head(page)[:2]:
            key=canonical_url(c.get("image_url"))
            if key and key not in seen:
                seen.add(key); c["score"] = 65 + (_image_relevance(story,c)*25) - (_image_penalty(c)*20); candidates.append(c)

    query=_image_story_query(story)
    source_domains={domain_of(x) for x in source_pages if x}
    for c in normal_image_search(query, num=IMAGE_SEARCH_RESULTS):
        key=canonical_url(c.get("image_url"))
        if key and key not in seen:
            seen.add(key)
            domain_bonus=15 if domain_of(c.get("page_url")) in source_domains else 0
            c["score"] = 50 + domain_bonus + (_image_relevance(story,c)*35) - (_image_penalty(c)*25)
            candidates.append(c)
...
def download_verified_image(candidate: dict) -> dict | None:
    """Fetch candidate bytes and verify HTTP 200, image MIME, size, decodability and Telegram geometry."""
    url=v1_network_url(text(candidate.get("image_url")))
    if not url or requests is None:
        return None
    try:
        # HEAD is only an early gate. Some CDNs reject HEAD, so a non-200 HEAD is not by itself a rejection.
        # The actual GET below is authoritative and must still return HTTP 200 + image/* + a bounded payload.
        h=session().head(url, headers=HEADERS, timeout=IMAGE_TIMEOUT, allow_redirects=True)
        hstatus=int(getattr(h, "status_code", 0) or 0)
        htype=text(getattr(h, "headers", {}).get("Content-Type")).split(";",1)[0].lower()
        hlen=text(getattr(h, "headers", {}).get("Content-Length"))
        if hstatus == 200 and hlen.isdigit() and int(hlen) > MAX_IMAGE_BYTES:
            logger.info("Image rejected HEAD size=%s url=%s", hlen, domain_of(url)); return None
        if hstatus == 200 and htype and (not htype.startswith("image/") or htype not in IMAGE_ALLOWED_MIME):
            logger.info("Image rejected non-supported-image HEAD content-type=%s url=%s", htype, domain_of(url)); return None
        if hstatus != 200:
            logger.info("Image HEAD status=%s; continuing with authoritative GET url=%s", hstatus, domain_of(url))
        final_url=text(getattr(h, "url", "")) or url

        r=session().get(final_url, headers=HEADERS, timeout=IMAGE_TIMEOUT, allow_redirects=True, stream=True)
        if int(getattr(r, "status_code", 0) or 0) != 200:
            logger.info("Image GET rejected status=%s url=%s", getattr(r, "status_code", 0), domain_of(final_url)); return None
        ctype=text(getattr(r, "headers", {}).get("Content-Type")).split(";",1)[0].lower()
        if not ctype.startswith("image/") or ctype not in IMAGE_ALLOWED_MIME:
            logger.info("Image GET rejected non-supported-image content-type=%s url=%s", ctype, domain_of(final_url)); return None
        length=text(getattr(r, "headers", {}).get("Content-Length"))
        if length.isdigit() and int(length) > MAX_IMAGE_BYTES:
            logger.info("Image rejected GET size=%s url=%s", length, domain_of(final_url)); return None

        suffix={"image/jpeg":".jpg","image/png":".png","image/webp":".webp"}.get(ctype, Path(urlsplit(final_url).path).suffix[:6] or ".img")
        if ctype == "image/gif" or suffix.lower()==".gif":
            logger.info("Image rejected unsupported animated/gif format url=%s", domain_of(final_url)); return None
        fd,path=tempfile.mkstemp(prefix="sn_photo_",suffix=suffix); os.close(fd)
        total=0
        try:
            ...
            with Image.open(path) as im:
                ...
                if max(width,height) / max(1,min(width,height)) > 20:
                    os.remove(path); return None
                if text(im.format).upper() not in {"JPEG","PNG","WEBP"}:
                    os.remove(path); return None
                im.verify()
            return {
                "path": path,
                "url": final_url,
                "source_page_url": text(candidate.get("page_url")),
                "source_name": text(candidate.get("source_name")) or domain_of(final_url),
                "provider": text(candidate.get("provider")),
                "bytes": total,
                "width": width,
                "height": height,
...
def source_label(url: str, fallback: str = "") -> str:
    ...
    if fb and len(fb)<=90 and not re.match(r"^https?://", fb, re.I) and not fb.startswith("a1ahr") and not re.fullmatch(r"[A-Za-z0-9_-]{40,}", fb):
        return fb
    return d or "Source"
...
def _source_records(sources: Any) -> list[dict]:
    ...
    for item in sources if isinstance(sources,list) else []:
        ...
        seen.add(cu)
        out.append({
            "name": source_label(url, raw_name),
            "url": url,
            "type": typ,
            "evidence_note": note,
        })
...
IMAGE_SEARCH_RESULTS = max(4, _env_int("IMAGE_SEARCH_RESULTS", 6))
...
IMAGE_MIN_RELEVANCE = _env_float("IMAGE_MIN_RELEVANCE", 0.30)
...
def _compact_live_value(value: str, limit: int = 34) -> str:
...
def live_rows(events: list[dict], kind: str) -> list[list[dict]]:
    """Build a compact two-column table for the live pair without synthetic match rows."""
    if kind == "next":
        rows=[[table_cell("GAME", True), table_cell("TIME", True, "center")]]
        for e in events[:10]:
            tm=to_bd(e["start"]).strftime("%H:%M") if e.get("start") else "TBC"
            rows.append([table_cell(_compact_live_value(_live_game_name(e))), table_cell(tm, align="center")])
        if len(rows)==1:
            rows.append([table_cell("No major games found"), table_cell("—", align="center")])
        return rows
    rows=[[table_cell("GAME", True), table_cell("RESULT", True, "center")]]
    for e in events[:10]:
        result=_live_result_value(e)
        if not result:
            continue
        rows.append([table_cell(_compact_live_value(_live_game_name(e))), table_cell(_compact_live_value(result,18), align="center")])
    if len(rows)==1:
        rows.append([table_cell("No major results found"), table_cell("—", align="center")])
    return rows

def live_rich(kind: str, target: date, events: list[dict]) -> dict:
    """Render the live post with a true Rich Message H1 and exactly two columns."""
    heading = "🏟️ NEXT-DAY GAME SCHEDULE" if kind == "next" else "🏅 PREVIOUS-DAY RESULTS"
    caption = f"{long_date(target)} · Asia/Dhaka"
    return {
        "blocks":[
            {"type":"heading", "size":1, "text":heading},
            {"type":"paragraph", "text":caption},
            {"type":"table", "cells":live_rows(events, kind), "is_bordered":True, "is_striped":True, "is_compact":True},
        ]
    }
...
def v1_architecture_tests() -> int:
    ...
    ck(IMAGE_ALLOWED_MIME == {"image/jpeg","image/png"}, "supported photo MIME contract")
    ...
    ck(IMAGE_ALLOWED_MIME == {"image/jpeg","image/png"}, "image MIME contract excludes unsupported webp")
    ...
    ck(len(rr) == 2 and rr[1][1]["text"] == "2–1", "results table contains only real results")
