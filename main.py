#!/usr/bin/env python3
# The Sports Newsroom V2.2 - normal-web-search-first evergreen + relevant original photo + rotating rich live sports pair for @TheSportsNewsroom.
#
# Design (see README.md):
#   V2: Normal web search discovers -> local coverage/dedupe -> Cerebras ranks -> Exa Contents verifies ->
...
def _image_relevance(story: dict, candidate: dict) -> float:
    ...
    hay = " ".join([
        ...
        text(candidate.get("alt")),
        text(candidate.get("page_title")),
        text(candidate.get("page_url")),
        text(candidate.get("source_name")),
    ])
    got = tokens(hay)
    if not wanted or not got:
        return 0.0
    return len(wanted & got) / max(1, len(wanted))
...
def _image_penalty(candidate: dict) -> float:
    ...
    return min(0.35, sum(0.10 for term in IMAGE_SEARCH_NOISE if term in hay))
...
IMAGE_MIN_RELEVANCE = _env_float("IMAGE_MIN_RELEVANCE", 0.10)
...
def live_rows(events: list[dict], kind: str) -> list[list[dict]]:
    ...
    for e in events[:12]:
        if e.get("style") == "team" and e.get("home") and e.get("away"):
            ...
            result=text(e.get("winner") or e.get("detail") or "Completed")
...
PROMPT_FALLBACKS = {
    "exa_sector_discovery_v1.txt": (
        "THE SPORTS NEWSROOM V2: NORMAL WEB SEARCH DISCOVERY\n\n"
...
def v1_state(state: dict) -> dict:
    ...
    # Re-assert the live-pair shape after legacy-state migration as the old v4 block may predate
    # the retirement queue. This keeps interrupted cleanup durable across the first migrated run.
    root["live"].setdefault("schedule", {"message_id": None, "target_date": ""})
    root["live"].setdefault("results", {"message_id": None, "target_date": ""})
    root["live"].setdefault("retiring_message_ids", [])
    return root
...
def normal_search_sector(sector: str, target: date, used_sectors: set[str], *, num: int | None=None) -> list[dict]:
    ...
    for item in rows:
        ...
        if exclusion_reason(alltext): continue
        if V1_CURRENT_TITLE_RX.search(title): continue
        out.append(c)
    return out
...
def _search_with_exa_fallback_sector(sector: str, target: date, used_sectors: set[str], *, num: int | None=None, hard: bool=False) -> list[dict]:
    ...
    if len(rows) >= NORMAL_SEARCH_FALLBACK_MIN:
        logger.info("V2 normal search primary sector=%s results=%d; Exa not used",sector,len(rows))
        return rows
    logger.warning("V2 normal search sparse sector=%s results=%d; using Exa fallback",sector,len(rows))
...
def v1_build_reservoir(raw: list[dict], coverage: dict, used_sectors: set[str]) -> tuple[list[dict], list[str]]:
    ...
    for c in ordered:
        ...
        c["sources"] = [{"name": c.get("source"), "url": network_url, "type": f"{text(c.get('search_provider')) or 'Web search'} result", "evidence_note": " ".join(c.get("highlights") or [])[:500]}]
...
def v1_rank(ai: "AIClient", candidates: list[dict], used_sectors: set[str]) -> list[dict]:
    ...
    system=_v1_prompt("cerebras_rank_v1.txt",
        "You are the ranking desk for The Sports Newsroom V2. Rank the 20-sector evergreen candidate set using only the supplied evidence. "
...
def v1_secondary_search(candidate: dict, target: date) -> list[dict]:
    ...
    if len(rows) < NORMAL_SEARCH_FALLBACK_MIN:
        logger.warning("V2 secondary normal search sparse; using Exa fallback")
...
def v1_normalize_story(story: dict) -> dict:
    ...
    # Legacy point lists are intentionally ignored. V2 editorial output is one paragraph.
...
def v1_validate_editorial(candidate: dict, editorial: dict) -> tuple[bool, str, dict]:
    """V2 editorial gate: preserve evergreen rules without inheriting overly broad legacy current-news matching."""
...
def v1_publish_evergreen(story: dict, media_path: str | None = None) -> dict:
    ...
    if not chosen_path:
        return {"ok":False,"description":"V2 evergreen publication failed: no verified photo and card generation failed"}
    try:
        payload={
            "chat_id":CHANNEL,
...
def rotate_live_pair(state: dict, vstate: dict, run_id: str,
    ...
    if not sr.get("ok"):
        logger.warning("New V2.2 live schedule failed; retaining previous schedule")
    if not rr.get("ok"):
        logger.warning("New V2.2 live results failed; retaining previous results")
    if dry:
...
def v1_run_once(mode: str = "live") -> int:
    ...
    ai=AIClient(state.get("ai"))
    if not ai.available:
        logger.error("CEREBRAS_API_KEY is required for V2 ranking/editorial generation"); return 1
    ...
    if missing:
        logger.info("V2 recovery: %d sectors need targeted discovery",len(missing))
    ...
    if missing:
        logger.warning("V2 discovery partial: %d/%d sectors remain uncovered this run: %s",len(missing),len(V1_SECTORS),", ".join(sorted(missing)))
    ranked=v1_rank(ai,reservoir,used_sectors) if reservoir else []
    if not ranked:
        logger.warning("V2 ranking produced no candidates; continuing to live pair only: %s", ai.last_error)
    ...
    if len(selected) < EVERGREEN_POSTS_PER_RUN:
        logger.info("V2 partial evergreen selection: %d/%d target posts available",len(selected),EVERGREEN_POSTS_PER_RUN)
    if deadline.expired():
    ...
    if status!="ok":
        logger.warning("V2 verification partial/unavailable: %s; publishing only candidates that still pass editorial validation",status)
    ...
    for cand in stream:
        ...
        if not editorial:
            logger.warning("V2 editorial generation failed for sector=%s subject=%s: %s",sec,text(cand.get("normalized_subject")),ai.last_error); continue
        ...
        if not dry:
            ...
            if isinstance(existing_pub,dict) and existing_pub.get("status")=="posted":
                logger.info("V2.2 evergreen publication already posted; skipping image research for %s",story.get("normalized_subject")); continue
        ...
        if not res.get("ok"): 
            if res.get("uncertain"):
                logger.error("V2 evergreen delivery uncertain for %s",story.get("headline")); break
            continue
        mid=(res.get("result") or {}).get("message_id")
    ...
    if len(posted) != EVERGREEN_POSTS_PER_RUN:
        logger.info("V2 evergreen phase partial: %d/%d target posts published; continuing to live pair",len(posted),EVERGREEN_POSTS_PER_RUN)
    try:
        sched_meta,res_meta,next_events,past_events=live_pair(state,now)
    except Exception as exc:
        logger.error("V2.2 live pair generation failed: %s",redact(str(exc))); return 1
    ...
    if not dry and not live_result.get("cleanup_ok"):
        logger.warning("V2.2 live pair published, but retired messages remain queued for deletion")
    ...
    logger.info("V2.2 RUN %s %s: %d evergreen + 2 live", "DRY" if dry else "SUCCESS", run_id, len(posted)); return 0
...
def v1_architecture_tests() -> int:
    ...
    print(f"V2 architecture tests: {checks-len(failures)}/{checks} passed")
...
def _run_diagnose() -> int:
    print(f"{APP_NAME} V2.2 diagnostic")
    failures=0
    if not TELEGRAM_BOT_TOKEN:
...
def v1_validate_config(require_secrets: bool=True) -> int:
    ...
    print(f"CONFIG: OK (The Sports Newsroom V2.2, {len(V1_SECTORS)} sectors, normal web search primary with Exa fallback)")
    return 0


def v1_main(argv: list[str] | None=None) -> int:
    global CHANNEL, DRY_RUN
    ap=argparse.ArgumentParser(description=f"{APP_NAME} V2.2 {APP_VERSION}")
    ...
    if args.version:
        print(f"{APP_NAME} V2.2 {APP_VERSION}"); return 0
