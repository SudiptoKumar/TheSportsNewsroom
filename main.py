...
def v1_architecture_tests() -> int:
    ...
    try:
        ...
        # Monkeypatch provider boundary without making network calls.
        called=[]
        old_req=globals().get("_v1_exa_request")
        def fake_req(url,body,**kw): called.append((url,body)); return type("R",(),{"ok":True,"status":200,"data":{"results":[]},"error":""})()
    ...
    # Contents payload contract
    called=[]; old_req=globals().get("_v1_exa_request")
    def fake_contents(url,body,**kw): called.append((url,body)); return type("R",(),{"ok":True,"status":200,"data":{"results":[],"statuses":[]},"error":""})()
    globals()["_v1_exa_request"]=fake_contents
    v1_contents_for_urls(["https://example.com/a","https://example.com/a/"],text_mode=True)
    ck(called and called[-1][0].endswith("/contents"),"contents endpoint")
    ck(len(called[-1][1]["urls"])==1,"contents URL dedupe")
    ck(called[-1][1]["maxAgeHours"]==720,"contents freshness policy")
    globals()["_v1_exa_request"]=old_req
    # Null-safety contract
    s=v1_normalize_story({"image":None,"sources":None,"key_points":None,"tags":None,"people":None})
    ck(isinstance(s["image"],dict),"null image normalized")
    ck(isinstance(s["sources"],list),"null sources normalized")
    # Search result -> candidate object
    row={"sector":"Rule Check","normalized_subject":"A Rule","central_knowledge_unit":"A Rule","central_claim":"highlight","source_url":"https://example.com","source":"Example","grade":"A","highlights":["highlight"],"text":"highlight","image":"https://example.com/a.jpg"}
    res=v1_build_reservoir([row],{"records":[]},set())[0]
    ck(bool(res) and res[0]["sources"] and res[0]["sources"][0]["url"]=="https://example.com/","candidate source contract")
...
def v1_self_test(fast: bool=False) -> int:
    # Keep stable legacy regression coverage, but suppress its mock-provider warning noise so production logs
    # do not make simulated fallback cases look like live Exa/Cerebras failures.
    previous_level=logger.level
    logger.setLevel(logging.ERROR)
    try:
        rc=v4_self_test(fast=fast)
    finally:
