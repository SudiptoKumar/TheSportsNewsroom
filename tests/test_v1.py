...
class TestV1Contracts(unittest.TestCase):
    ...
    def test_release_workflow_is_version_locked_and_state_safe(self):
        ...
        self.assertIn('test "$(python main.py --version)" = "The Sports Newsroom V1 1.3.0"', text)
    ...
    def test_single_pipeline_entrypoints(self):
        self.assertEqual("1.3.0", main.APP_VERSION)
...
class TestV1SearchAndCard(unittest.TestCase):
    def test_v1_version_is_single_public_version(self):
        self.assertEqual("1.3.0", main.APP_VERSION)
        source = Path(main.__file__).read_text(encoding="utf-8")
        self.assertIn('APP_VERSION = "1.3.0"', source)
    ...
    def test_live_rows_always_have_exactly_two_columns_with_empty_state(self):
        next_rows=main.live_rows([],"next")
        result_rows=main.live_rows([],"past")
        self.assertTrue(all(len(row)==2 for row in next_rows))
        self.assertTrue(all(len(row)==2 for row in result_rows))
