...
class OfflineProviderIsolationMixin:
    def setUp(self):
        self._provider_keys = {
            "EXA_API_KEY": main.EXA_API_KEY,
            "CEREBRAS_API_KEY": main.CEREBRAS_API_KEY,
            "TELEGRAM_BOT_TOKEN": main.TELEGRAM_BOT_TOKEN,
            "CRICKETDATA_API_KEY": main.CRICKETDATA_API_KEY,
        }
        main.EXA_API_KEY = ""
        main.CEREBRAS_API_KEY = ""
        main.TELEGRAM_BOT_TOKEN = ""
        main.CRICKETDATA_API_KEY = ""

    def tearDown(self):
        for name, value in self._provider_keys.items():
            setattr(main, name, value)

class TestV1Contracts(OfflineProviderIsolationMixin, unittest.TestCase):
    def test_exact_twenty_sectors(self):
        self.assertEqual(20, len(main.V1_SECTORS))
        self.assertEqual(20, len(set(main.V1_SECTORS)))

    def test_release_workflow_is_version_locked_and_state_safe(self):
        wf=Path(main.__file__).parent / ".github" / "workflows" / "newbot.yml"
        text=wf.read_text(encoding="utf-8")
        self.assertIn('RUN_DEADLINE_SECONDS: "1500"', text)
        self.assertIn("TELEGRAM_CHANNEL_NAME: ${{ vars.TELEGRAM_CHANNEL_NAME || 'Sports News' }}", text)
        self.assertIn("TELEGRAM_CHANNEL_URL: ${{ vars.TELEGRAM_CHANNEL_URL || 'https://t.me/TheSportsNewsroom' }}", text)
        self.assertIn('test "$(python main.py --version)" = "The Sports Newsroom V1 1.4.0"', text)
        self.assertNotIn("prompts/exa_contents_verification_v1.txt", text)
        importer=(Path(main.__file__).parent / ".github" / "workflows" / "import-zip.yml").read_text(encoding="utf-8")
        self.assertIn('--exclude="news_state.json"', importer)
        self.assertIn('--exclude="coverage_index.json"', importer)
    ...
    def test_editorial_prompt_requests_one_40_to_60_word_paragraph(self):
        ...
        for current in (prompt, file_prompt):
            self.assertIn("ONE short paragraph of 40-60 words", current)
            self.assertIn("Do not use lists", current)
            self.assertIn("Prefer 2-4 natural sentences", current)
            self.assertIn("exactly 2 relevant, topic-specific hashtags", current)
    ...
    def test_reservoir_rejects_definition_noise_before_ai_ranking(self):
        raw=[
            {"sector":"Sport Discovery","normalized_subject":"The Meaning of Unusual","source_url":"https://www.merriam-webster.com/dictionary/unusual","source":"Merriam-Webster","grade":"B","highlights":["definition and meaning"],"text":"dictionary definition of unusual","image":""},
            {"sector":"Sport Discovery","normalized_subject":"Football history fact","source_url":"https://www.bbc.com/sport/football/example","source":"BBC Sport","grade":"B","highlights":["football history and rules"],"text":"A documented football sports history fact.","image":""},
        ]
        reservoir,_=main.v1_build_reservoir(raw,{"records":[]},set())
        self.assertEqual(1,len(reservoir))
        self.assertEqual("Football history fact",reservoir[0]["normalized_subject"])

    def test_verification_unavailable_cannot_bypass_to_editorial(self):
        fixed_now=datetime(2026,9,22,10,0,tzinfo=main.BD_TZ)
        state={"posts":[]}
        candidates=[{"sector":main.V1_SECTORS[0],"normalized_subject":"Football History","central_knowledge_unit":"Football history","central_claim":"A verified football claim","topic":"Football History","source_url":"https://example.com/football","source":"Example","grade":"B","highlights":["football evidence"],"image":"","sources":[{"name":"Example","url":"https://example.com/football","grade":"B"}],"research_images":[]}]
        ranked=[{"post_number":1,"score":100,"reason":"","_candidate_pool":candidates}]
        with patch.object(main,"CEREBRAS_API_KEY","test"), patch.object(main,"TELEGRAM_BOT_TOKEN","test"),              patch.object(main,"now_bd",return_value=fixed_now), patch.object(main,"load_state",return_value=state),              patch.object(main,"load_coverage",return_value={"records":[]}), patch.object(main,"v1_discover_all_sectors",return_value=candidates),              patch.object(main,"v1_rank",return_value=ranked), patch.object(main,"v1_verify_selected",return_value=([],"no_verified_candidates")),              patch.object(main,"v1_editorialize",side_effect=AssertionError("editorial must not run without verification")),              patch.object(main,"live_pair",return_value=({"target_date":"2026-09-23"},{"target_date":"2026-09-21"},[],[])):
            rc=main.run_once(mode="dry-run")
        self.assertEqual(0,rc)
        self.assertEqual([], [p for p in state.get("posts",[]) if p.get("desk")=="evergreen_v1"])

    def test_source_normalization_rejects_internal_ids_and_keeps_real_names(self):
        rows=main._source_records([
            {"name":"a1ahr0chm6ly93d3c...","url":"a1ahr0chm6ly93d3c..."},
            {"name":"BBC Sport","url":"https://www.bbc.co.uk/sport/football/example"},
        ])
        self.assertEqual(1, len(rows))
        self.assertEqual("BBC Sport", rows[0]["name"] if isinstance(rows[0],dict) else rows[0][0])
        self.assertTrue(rows[0]["url"] if isinstance(rows[0],dict) else rows[0][1])
    ...
    def test_v1_state_reasserts_live_retirement_queue_after_legacy_migration(self):
        ...
        self.assertEqual([11,12], live["retiring_message_ids"])
        self.assertEqual(11, live["schedule"]["message_id"])
        self.assertEqual(12, live["results"]["message_id"])

    def test_live_rich_has_h1_and_exactly_three_columns(self):
        ...
        self.assertTrue(all(len(row) == 3 for row in table["cells"]))
        self.assertEqual(["SPORT", "GAME", "TIME"], [c["text"] for c in table["cells"][0]])
    ...
    def test_live_rich_results_uses_game_and_result_columns(self):
        ...
        table = rich["blocks"][2]
        self.assertEqual(["SPORT", "GAME", "RESULT"], [c["text"] for c in table["cells"][0]])
    ...
    def test_render_evergreen_post_exact_html_contract(self):
        ...
        self.assertIn(f'<b><a href="{main.CHANNEL_URL}">{main.CHANNEL_DISPLAY_NAME}</a></b>', html)
        self.assertIn("<b><a href=\"https://t.me/TheSportsNewsroom\">Sports News</a></b> #Football #History", lines)
        self.assertEqual("Source: <a href=\"https://example.com/story\">Example Source</a>", lines[-1])
        self.assertNotIn("https://example.org/item", html)
        old_channel=main.CHANNEL
        old_url=main.CHANNEL_URL
        old_name=main.CHANNEL_DISPLAY_NAME
        old_handle=main.CHANNEL_HANDLE
        try:
            main.CHANNEL="@TestSportsChannel"
            main.CHANNEL_DISPLAY_NAME="The Sports Newsroom Test"
            main.CHANNEL_HANDLE="@TestSportsChannel"
            override_html=main.render_evergreen_post(story)["html"]
            self.assertIn('<a href="https://t.me/TestSportsChannel">TestSportsChannel</a>', override_html)
            self.assertIn('<b><a href="https://t.me/TestSportsChannel">TestSportsChannel</a></b> #Football #History', override_html)
        finally:
            main.CHANNEL=old_channel; main.CHANNEL_URL=old_url; main.CHANNEL_DISPLAY_NAME=old_name; main.CHANNEL_HANDLE=old_handle
    ...
    def test_unknown_source_uses_readable_site_name(self):
        self.assertEqual("Example Sports Site", main.source_public_name("https://example-sports-site.com/story"))
        self.assertEqual("The Guardian", main.source_public_name("https://www.theguardian.com/sport/story"))
    ...
    def test_verified_image_requires_200_image_mime_and_preserves_dimensions(self):
        ...
        buf = BytesIO()
        Image.new("RGB", (641, 479), (70, 100, 160)).save(buf, format="JPEG", quality=91)
        ...
        candidate={"image_url":"https://img.example/original.jpg","page_url":"https://example.com/story","title":"Example football history photo","alt":"football history","source_name":"Example","provider":"bing_images","score":90}
    ...
    def test_verified_image_allows_head_not_supported_when_get_is_valid(self):
        from io import BytesIO
        from PIL import Image
        buf=BytesIO(); Image.new("RGB",(500,300),(70,100,160)).save(buf,format="JPEG",quality=90); payload=buf.getvalue()
        class HeadResp:
    ...
    def test_external_image_failure_falls_back_to_branded_card(self):
        ...
        with patch.object(main,"discover_evergreen_image_candidates",return_value=[]), patch.object(main,"make_card",return_value="/tmp/fallback-card.jpg"), patch.object(main,"v1_exa_search_query",side_effect=AssertionError("offline fallback test must not call Exa")):
            path,meta=main.prepare_evergreen_media(story, allow_exa_fallback=False)
    ...
    def test_exa_image_fallback_can_be_explicitly_disabled(self):
        story={"headline":"Example sports story","body":EDITORIAL_BODY,"sources":[],"tags":["#Sports"],"image":{},"image_status":"pending"}
        with patch.object(main,"discover_evergreen_image_candidates",return_value=[]), patch.object(main,"v1_exa_search_query",side_effect=AssertionError("disabled Exa fallback must not run")), patch.object(main,"make_card",return_value="/tmp/fallback-card.jpg"), patch.object(main,"EXA_API_KEY","configured"):
            path,meta=main.prepare_evergreen_media(story, allow_exa_fallback=False)
        self.assertEqual("/tmp/fallback-card.jpg",path)
        self.assertEqual("card_fallback",story["image_status"])
        self.assertEqual("card_fallback",meta["mode"])
    ...
    def test_build_evergreen_strips_legacy_editorial_fields_and_caps_two_tags(self):
        ...
        self.assertEqual(["#One", "#Two"], story["tags"])
    ...
    def test_editorial_validator_rejects_more_than_two_hashtags(self):
        candidate = {"sources": [{"name": "Source", "url": "https://example.com"}], "central_claim": "Football laws were standardized", "research_evidence": "laws were standardized"}
    ...
    def test_editorial_validator_rejects_ai_style_dash_punctuation(self):
        candidate = {"sources": [{"name": "Source", "url": "https://example.com"}], "central_claim": "Football laws were standardized", "research_evidence": "laws were standardized"}
        em = dict(EDITORIAL_SAMPLE); em["body"] = EDITORIAL_BODY.replace("Football's", "Football's —")
        ok, why, _ = main.v1_validate_editorial(candidate, em)
        self.assertFalse(ok)
        self.assertEqual("em_dash_style", why)

    def test_editorial_prompt_requires_human_natural_style_without_dash_punctuation(self):
        prompt = Path("prompts/cerebras_editorial_v1.txt").read_text(encoding="utf-8")
        self.assertIn("human-sounding", prompt)
        self.assertIn("40-60 words", prompt)
        self.assertIn("Do not use an em dash (—) or en dash (–)", prompt)
        self.assertIn("Avoid hype, clichés, marketing language", prompt)
    ...
    def test_production_orchestrator_dry_run_reaches_live_pair(self):
        ...
        for i, sec in enumerate(main.V1_SECTORS):
            candidates.append({
                "sector": sec, "normalized_subject": f"Football History Subject {i}", "central_knowledge_unit": f"Football knowledge {i}",
                "central_claim": "A verified football sports claim", "topic": f"Football History Subject {i}",
                "source_url": f"https://example.com/{i}", "source": "Example", "grade": "A",
                "highlights": ["Verified football sports evidence about this subject."], "image": "",
    ...
    def test_partial_evergreen_volume_does_not_block_live_pair(self):
        ...
        for i, sec in enumerate(main.V1_SECTORS[:5]):
            candidates.append({
                ...
                "highlights": ["Verified football sports evidence about this subject."], "image": "",
                "sources": [{"name": "Example", "url": f"https://example.com/{i}", "grade": "A"}],
                "research_images": [], "research_evidence": "Verified football sports evidence about this subject.",
            })
...
class TestV1SearchAndCard(OfflineProviderIsolationMixin, unittest.TestCase):
    def test_v1_version_is_single_public_version(self):
        self.assertEqual("1.4.0", main.APP_VERSION)
        source = Path(main.__file__).read_text(encoding="utf-8")
        self.assertIn('APP_VERSION = "1.4.0"', source)

    def test_evergreen_channel_link_is_bold_and_same_line_as_two_hashtags(self):
        story={"headline":"A Sporting Story With A Clear History","body":EDITORIAL_BODY,"sources":[("Source","https://example.com/story")],"tags":["#Chaupar","#GameHistory"],"image":{}}
        rendered=main.render_evergreen_post(story)["html"]
        lines=rendered.splitlines()
        channel_line=next((x for x in lines if "Sports News" in x and "#Chaupar" in x), "")
        self.assertEqual('<b><a href="https://t.me/TheSportsNewsroom">Sports News</a></b> #Chaupar #GameHistory', channel_line)
        self.assertNotIn("@TheSportsNewsroom", channel_line)

    def test_live_rows_always_have_exactly_three_columns_with_empty_state(self):
        next_rows=main.live_rows([],"next")
        result_rows=main.live_rows([],"past")
        self.assertTrue(all(len(row)==3 for row in next_rows))
        self.assertTrue(all(len(row)==3 for row in result_rows))
        self.assertIn("No major games found", json.dumps(next_rows, ensure_ascii=False))
        self.assertIn("No major results found", json.dumps(result_rows, ensure_ascii=False))

    def test_live_rich_uses_requested_schedule_and_results_headings(self):
        self.assertIn("NEXT-DAY GAME SCHEDULE", main.live_rich("next", date(2026,9,24), [])["blocks"][0]["text"])
        self.assertIn("PREVIOUS-DAY RESULTS", main.live_rich("past", date(2026,9,22), [])["blocks"][0]["text"])

    def test_results_rows_require_real_result_values(self):
        final=main.make_event(sport="Football", league="Premier League", home="A", away="B", name="A vs B", start=datetime(2026,9,22,18,tzinfo=timezone.utc), state="final", home_score="2", away_score="1")
        missing=main.make_event(sport="Football", league="Premier League", home="C", away="D", name="C vs D", start=datetime(2026,9,22,18,tzinfo=timezone.utc), state="final", home_score="", away_score="")
        self.assertEqual("2–1", main._live_result_value(final))
        self.assertEqual("", main._live_result_value(missing))
        self.assertEqual(0, len(main.major_live_events([missing], "past", date(2026,9,22))))

    def test_live_rows_include_sport_column_and_game_name(self):
        event=main.make_event(sport="Basketball",league="NBA",home="New York Liberty",away="Atlanta Dream",name="New York Liberty vs Atlanta Dream",start=datetime(2026,9,24,18,tzinfo=timezone.utc),state="scheduled")
        rows=main.live_rows([event],"next")
        self.assertEqual(["SPORT","GAME","TIME"],[c["text"] for c in rows[0]])
        self.assertEqual("Basketball",rows[1][0]["text"])
        self.assertIn("New York Liberty",rows[1][1]["text"])

    def test_live_rich_does_not_expose_internal_timezone(self):
        rich=main.live_rich("next",date(2026,9,24),[])
        self.assertNotIn("Asia/Dhaka",json.dumps(rich,ensure_ascii=False))

    def test_tsdb_events_checks_adjacent_days_for_schedule_recovery(self):
        calls=[]
        def fake_fetch(d):
            calls.append(d)
            return []
        with patch.object(main,"tsdb_fetch",side_effect=fake_fetch):
            main.tsdb_events(date(2026,9,24))
        self.assertEqual([date(2026,9,23),date(2026,9,24),date(2026,9,25)],calls)

    def test_live_pair_uses_previous_and_next_dhaka_dates(self):
        now=datetime(2026,9,23,10,tzinfo=main.BD_TZ)
        tomorrow=now.date()+timedelta(days=1); yesterday=now.date()-timedelta(days=1)
        next_ev=main.make_event(sport="Football", league="Premier League", home="A", away="B", name="A vs B", start=datetime(2026,9,24,18,tzinfo=main.BD_TZ), state="scheduled", tier="S")
        past_ev=main.make_event(sport="Football", league="Premier League", home="C", away="D", name="C vs D", start=datetime(2026,9,22,18,tzinfo=main.BD_TZ), state="final", tier="S", home_score="2", away_score="1")
        with patch.object(main, "collect_events", side_effect=[([next_ev],"ok"),([past_ev],"ok")]):
            meta_next,meta_prev,ne,pe=main.live_pair({},now)
        self.assertEqual(tomorrow.isoformat(), meta_next["target_date"])
        self.assertEqual(yesterday.isoformat(), meta_prev["target_date"])
        self.assertEqual(1,len(ne)); self.assertEqual(1,len(pe))
    ...
    def test_normal_search_is_primary_and_does_not_call_exa_when_sufficient(self):
        html='''<a class="result__a" href="https://example.com/a">Example Football History</a><a class="result__snippet">A documented football history story about the sport and its rules.</a>'''
        calls=[]
    ...
    def test_v1_normal_search_does_not_require_exa_secret_for_config(self):
