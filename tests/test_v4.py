...
class V4ArchitectureTests(unittest.TestCase):
    ...
    def test_exa_package_validator_enforces_twenty_sectors(self):
        ...
        for index, sector in enumerate(app.V4_SECTORS, 1):
            ...
            posts.append({
                "sector": sector,
                "topic": f"{token} reference",
                "sport_or_game": token,
                "normalized_subject": token,
                "central_knowledge_unit": f"{token} knowledge",
                "central_claim": f"{token} has a distinct documented history.",
                "editorial_angle": "explanation",
                "headline": f"A verified evergreen note about {token} history",
                "main_story": "A permanent historical or rules-based explanation supported by the supplied source material.",
                "source_1": {"name": "Archive", "url": f"https://example.com/{index}", "type": "archive", "evidence_note": "Verified source material."},
                "source_2": None,
                "source_3": None,
                "sources": [],
                "image": {"image_url": "", "source_page_url": "", "source_name": "", "image_status": "unavailable"},
            })
